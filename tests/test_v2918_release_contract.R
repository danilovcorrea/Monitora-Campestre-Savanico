...
script <- normalizePath(
  if (length(args)) args[[1L]] else "monitora_campsav_alvo_global_v2.9.18.R",
  mustWork = TRUE
)
baseline <- normalizePath(
  if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.17.R",
  mustWork = TRUE
)
...
exigir(
  ...
  "A seção congelada do RStudio anterior às variáveis manuais divergiu da v2.9.17."
)
...
exigir(!grepl("2.9.18-dev", texto, fixed = TRUE), "Marcador dev permaneceu no script público.")
...
for (arquivo in c(
  "README.md", "CHANGELOG.md", "GUIA_USUARIO_v2.9.18.md",
  "RELEASE_NOTES_v2.9.18.md", "VERSION",
  "monitora_campsav_alvo_global.R", "R_monitora_campsav_alvo_global.R",
  "R/monitora_campsav_alvo_global.R",
  "releases/v2.9.18/SHA256SUMS.txt",
  "release_assets/v2.9.18/SHA256SUMS.txt"
)) exigir(file.exists(arquivo), paste0("Arquivo de release ausente: ", arquivo))

exigir(
  identical(trimws(readLines("VERSION", warn = FALSE)), "2.9.18"),
  "VERSION não contém exatamente 2.9.18."
)
hashes <- unname(tools::md5sum(c(
  ...
  "R/monitora_campsav_alvo_global.R",
  "releases/v2.9.18/monitora_campsav_alvo_global_v2.9.18.R",
  "release_assets/v2.9.18/monitora_campsav_alvo_global_v2.9.18.R"
)))
exigir(length(unique(hashes)) == 1L, "Cópias públicas do script não são byte a byte idênticas.")

cat("TEST_V2918_RELEASE_CONTRACT_OK\n")
