...
baseline <- normalizePath(
  if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.21.R",
  winslash = "/", mustWork = TRUE
)
...
exigir(length(arvore_c) == 1L && identical(arvore_c[[1L]][[1L]], quote(base::evalq)),
...
exigir(identical(inicio_congelado(linhas_c), inicio_congelado(linhas_b)),
  "A inicialização congelada do RStudio divergiu da v2.9.21 publicada.")
...
exigir(identical(payload_c, payload_b) && identical(hex_c, hex_b),
  "O código que materializa o contrato XLSForm embutido foi alterado.")
env_c <- new.env(parent = globalenv()); env_b <- new.env(parent = globalenv())
...
exigir(
  ...
  "O contrato XLSForm materializado divergiu da v2.9.21 publicada."
)

versao_esperada <- if (grepl("-dev", basename(candidata), fixed = TRUE)) "2.9.22-dev" else "2.9.22"
build_esperado <- if (identical(versao_esperada, "2.9.22-dev")) {
  "v2.9.22-dev-20260831-r02"
} else "v2.9.22-20260831-r02"
for (trecho in c(
  paste0('MONITORA_SCRIPT_VERSAO <- "', versao_esperada, '"'),
  paste0('MONITORA_SCRIPT_BUILD_ID <- "', build_esperado, '"'),
  ...
  'monitora_contrato_tokens_invalidos_valor('
)) exigir(grepl(trecho, texto_c, fixed = TRUE), paste0("Requisito v2.9.22 ausente: ", trecho))
...
cat(sprintf(
  paste0("TEST_V2922_INTEGRIDADE_CONTRATO_RELEASE_OK; contrato_identico=TRUE; ",
