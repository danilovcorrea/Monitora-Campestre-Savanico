...
candidato <- normalizePath(
  if (length(args) >= 1L) args[[1L]] else
    "monitora_campsav_alvo_global_v2.9.9.R",
  mustWork = TRUE
)
...
resultado <- list(
  ...
  v299 = simular_source_com_eco(candidato)
)

stopifnot(
  resultado$v291$expressoes_externas == 1L,
  resultado$v299$expressoes_externas == 1L,
  resultado$v291$eco_bytes <= 512,
  resultado$v299$eco_bytes <= 512,
  identical(resultado$v291$eco_bytes, resultado$v299$eco_bytes)
)
...
cat("TEST_V299_INICIALIZACAO_RSTUDIO_OK\n")
