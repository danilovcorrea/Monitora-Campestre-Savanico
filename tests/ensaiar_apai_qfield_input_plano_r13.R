...
registros <- fread(file.path(produto, "01_produtos_dados", "registros_corrig_stat.csv"), encoding = "UTF-8")
validacao <- fread(file.path(produto, "04_validacao_espacial", "pos_painel", "validacao_espacial_coletas.csv"), encoding = "UTF-8")
consensos <- fread(file.path(produto, "04_validacao_espacial", "pos_painel", "validacao_espacial_consenso_ua.csv"), encoding = "UTF-8")
...
resultado <- env$monitora_qfield_gerar(
  ...
  origem_ensaio = "APAI: simulação de usuário com qfield_input plano; candidata r13",
...
cat(sprintf(
  paste0("APAI_QFIELD_INPUT_PLANO_R13_OK; zoom=%d; UAs=%d; tiles=%d/%d; ",
