...
script <- normalizePath(
  if (length(args)) args[[1L]] else "monitora_campsav_alvo_global_v2.9.20.R",
  mustWork = TRUE
)
baseline <- normalizePath(
  if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.19.R",
  mustWork = TRUE
)
...
exigir(
  ...
  "A seção congelada do RStudio anterior às variáveis manuais divergiu da v2.9.19."
)
...
for (trecho in c(
  ...
  'monitora_doc_validacao_auditar_md <- function',
  'monitora_doc_validacao_auditar_docx <- function',
  'monitora_doc_validacao_adequar_layout_docx <- function',
  ...
  'monitora_correcao_contrato_fechamento_hierarquico <- function',
  'status_resolucao_contexto',
  'monitora_relatorios_analiticos_cache_sentinel_compativel <- function',
  'OVERVIEW_LEVEL=',
  'contrato_overview_valido',
  'cache_contrato_fonte_validado',
  'monitora_painel_reconciliar_fila_semantica(solicitadas, gravar = TRUE, bloquear = FALSE)',
  'antes de atualizar a prévia integral ou salvar'
)) exigir(grepl(trecho, texto, fixed = TRUE), paste0("Revisão focal ausente: ", trecho))

for (trecho_proibido in c(
  "GTIFF_DIR",
  "terra::ext(overview) <-",
  "terra::crs(overview) <-",
  "if (rec_pos_exclusao$n_conflitos > 0L) return(NULL)"
)) exigir(!grepl(trecho_proibido, texto, fixed = TRUE), paste0(
  "Implementação regressiva permaneceu: ", trecho_proibido
))
...
exigir(!grepl("2.9.20-dev", texto, fixed = TRUE), "Marcador dev permaneceu no script público.")
...
for (arquivo in c(
  "README.md", "CHANGELOG.md", "GUIA_USUARIO_v2.9.20.md",
  "RELEASE_NOTES_v2.9.20.md", "VERSION",
  "monitora_campsav_alvo_global.R", "R_monitora_campsav_alvo_global.R",
  "R/monitora_campsav_alvo_global.R",
  "releases/v2.9.20/SHA256SUMS.txt",
  "release_assets/v2.9.20/SHA256SUMS.txt"
)) exigir(file.exists(arquivo), paste0("Arquivo de release ausente: ", arquivo))

exigir(
  identical(trimws(readLines("VERSION", warn = FALSE)), "2.9.20"),
  "VERSION não contém exatamente 2.9.20."
)
hashes <- unname(tools::md5sum(c(
  ...
  "R/monitora_campsav_alvo_global.R",
  "releases/v2.9.20/monitora_campsav_alvo_global_v2.9.20.R",
  "release_assets/v2.9.20/monitora_campsav_alvo_global_v2.9.20.R"
)))
exigir(length(unique(hashes)) == 1L, "Cópias públicas do script não são byte a byte idênticas.")

cat("TEST_V2920_RELEASE_CONTRACT_OK\n")
