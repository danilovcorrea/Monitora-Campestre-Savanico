...
script <- normalizePath(
  if (length(args)) args[[1L]] else "monitora_campsav_alvo_global_v2.9.15.R",
  mustWork = TRUE
)
baseline <- normalizePath(
  if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.14.R",
  mustWork = TRUE
)
...
exigir(
  ...
  "A seção congelada do RStudio anterior às variáveis manuais divergiu da v2.9.14."
)
...
tmp <- tempfile("v2915_completude_")
...
exigir(!grepl("2.9.15-dev", texto, fixed = TRUE), "Marcador dev permaneceu no script público.")
...
for (arquivo in c(
  "README.md", "CHANGELOG.md", "GUIA_USUARIO_v2.9.15.md",
  "RELEASE_NOTES_v2.9.15.md", "VERSION",
  "monitora_campsav_alvo_global.R", "R_monitora_campsav_alvo_global.R",
  "R/monitora_campsav_alvo_global.R",
  "releases/v2.9.15/SHA256SUMS.txt",
  "release_assets/v2.9.15/SHA256SUMS.txt"
)) exigir(file.exists(arquivo), paste0("Arquivo de release ausente: ", arquivo))

exigir(
  identical(trimws(readLines("VERSION", warn = FALSE)), "2.9.15"),
  "VERSION não contém exatamente 2.9.15."
)
hashes <- unname(tools::md5sum(c(
  ...
  "R/monitora_campsav_alvo_global.R",
  "releases/v2.9.15/monitora_campsav_alvo_global_v2.9.15.R",
  "release_assets/v2.9.15/monitora_campsav_alvo_global_v2.9.15.R"
)))
exigir(length(unique(hashes)) == 1L, "Cópias públicas do script não são byte a byte idênticas.")

cat("TEST_V2915_RELEASE_CONTRACT_OK\n")
