...
candidato <- normalizePath(
  if (length(args) >= 1L) args[[1L]] else
    "monitora_campsav_alvo_global_v2.9.8.R",
  mustWork = TRUE
)
...
resultado <- list(
  ...
  v298 = simular_source_com_eco(candidato)
)

stopifnot(
  resultado$v291$expressoes_externas == 1L,
  resultado$v298$expressoes_externas == 1L,
  resultado$v291$eco_bytes <= 512,
  resultado$v298$eco_bytes <= 512,
  identical(resultado$v291$eco_bytes, resultado$v298$eco_bytes)
)
...
cat("TEST_V298_INICIALIZACAO_RSTUDIO_OK\n")
