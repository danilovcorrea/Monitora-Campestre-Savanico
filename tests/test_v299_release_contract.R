...
candidato <- normalizePath(if (length(args)) args[[1L]] else "monitora_campsav_alvo_global_v2.9.9.R", mustWork = TRUE)
baseline <- normalizePath(if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.8.R", mustWork = TRUE)
...
exigir(identical(inicio(linhas), inicio(base)),
       "A seção de inicialização anterior às variáveis manuais divergiu da v2.9.8.")
...
)) exigir(grepl(trecho, texto, fixed = TRUE), paste0("Contrato v2.9.9 ausente: ", trecho))

exigir(!grepl("2.9.9-dev", texto, fixed = TRUE), "Marcador dev permaneceu no script público.")
...
for (arquivo in c("README.md", "CHANGELOG.md", "GUIA_USUARIO_v2.9.9.md", "RELEASE_NOTES_v2.9.9.md", "VERSION"))
  exigir(file.exists(arquivo), paste0("Arquivo de release ausente: ", arquivo))

cat("TEST_V299_RELEASE_CONTRACT_OK\n")
