...
candidata <- normalizePath(
  if (length(args)) args[[1L]] else "monitora_campsav_alvo_global_v2.9.25.R",
  mustWork = TRUE
)
baseline <- normalizePath(
  if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.24.R",
  mustWork = TRUE
)
...
assert(identical(inicio_congelado(linhas_c), inicio_congelado(linhas_b)),
  "A inicialização congelada do RStudio divergiu da v2.9.24 pública.")
...
assert(
  ...
  "O contrato XLSForm materializado divergiu da v2.9.24 pública."
)
...
for (trecho in c(
  ...
  'monitora_relatorios_analiticos_material_documentado'
...
cat(sprintf(
    paste0("TEST_V2925_INTEGRIDADE_CONTRATO_RELEASE_OK; contrato_identico=TRUE; ",
