...
candidata <- normalizePath(
  if (length(args)) args[[1L]] else "monitora_campsav_alvo_global_v2.9.24.R",
  mustWork = TRUE
)
baseline <- normalizePath(
  if (length(args) >= 2L) args[[2L]] else "monitora_campsav_alvo_global_v2.9.23.R",
  mustWork = TRUE
)
...
assert(identical(inicio_congelado(linhas_c), inicio_congelado(linhas_b)),
  "A inicialização congelada do RStudio divergiu da v2.9.23 pública.")
...
assert(
  ...
  "O contrato XLSForm materializado divergiu da v2.9.23 pública."
)

texto <- paste(linhas_c, collapse = "\n")
for (trecho in c(
  'MONITORA_SCRIPT_VERSAO <- "2.9.24"',
  'MONITORA_SCRIPT_BUILD_ID <- "v2.9.24-20260909-r01"',
  'monitora_io_selecionar_aba_xlsx',
  'monitora_correcao_vincular_representacoes_contratuais_painel',
  'monitora_stat_reconciliar_grafias_uc',
  'monitora_fread_preserva_aspas_csv',
  'monitora_registros_corrig_ler_csv_normalizado'
...
cat(sprintf(
    paste0("TEST_V2924_INTEGRIDADE_CONTRATO_RELEASE_OK; contrato_identico=TRUE; ",
