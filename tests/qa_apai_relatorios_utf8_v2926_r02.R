...
ler <- function(rel, obrigatorio = TRUE) {
  ...
  if (!file.exists(p) && startsWith(rel, "05_estatisticas/")) {
    p <- file.path(fonte, basename(rel))
  }
  if (!file.exists(p)) {
...
if (isTRUE(gate_utf8_v2926_r02)) {
  ...
  assert(length(esperados_utf8) == 8L, "O gate UTF-8 não localizou os seis documentos e dois índices esperados.")
  assert(all(file.exists(esperados_utf8) & file.info(esperados_utf8)$size > 0L),
    "O gate UTF-8 não materializou todos os relatórios e índices esperados.")
  textos_utf8 <- list.files(dir_rel, pattern = "[.](csv|txt|md|Rmd|html|json)$", recursive = TRUE, full.names = TRUE)
  invalidos_utf8 <- textos_utf8[!vapply(textos_utf8, function(arq) {
    bruto <- readBin(arq, "raw", n = file.info(arq)$size)
    validUTF8(rawToChar(bruto))
  }, logical(1L))]
  assert(!length(invalidos_utf8), paste0("Produtos textuais inválidos em UTF-8: ", paste(basename(invalidos_utf8), collapse = " | ")))
