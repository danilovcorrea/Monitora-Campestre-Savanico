...
# vida de plantas columns
vida_cols <- colnames(registros_corrig)[
  str_detect(colnames(registros_corrig), "vida de plantas")
]

# presença herbácea e lenhosa
registros_herb_presence <- registros_corrig %>%
  mutate(
    herb_presence = apply(
      select(., all_of(vida_cols)), 1,
      function(row) any(str_detect(row, herb_pattern))
    )
  )

registros_lenh_presence <- registros_corrig %>%
  mutate(
    lenh_presence = apply(
      select(., all_of(vida_cols)),
      1,
      function(row) any(str_detect(row, lenh_pattern))
    )
  )

# Summarise by transect (UC, UA, ANO)
herb_summary <- registros_herb_presence %>%
  group_by(UC, UA, ANO) %>%
  summarise(sum_presence_herb = sum(herb_presence, na.rm = TRUE), .groups = "drop")

lenh_summary <- registros_lenh_presence %>%
  group_by(UC, UA, ANO) %>%
  summarise(sum_presence_lenh = sum(lenh_presence, na.rm = TRUE), .groups = "drop")

# Merge with registros_corrig_stat
registros_corrig_stat <- registros_corrig_stat %>%
  left_join(herb_summary, by = c("UC", "UA", "ANO")) %>%
  relocate(sum_presence_herb, .before = sum_herbacea)

registros_corrig_stat <- registros_corrig_stat %>%
  left_join(lenh_summary, by = c("UC", "UA", "ANO")) %>%
  relocate(sum_presence_lenh, .before = sum_herbacea)

# encostam na vareta column
encostam_vareta_col <- colnames(registros_corrig)[
  str_detect(colnames(registros_corrig), "Encostam.+vareta")
]

# presence nativa, exotica, seca_morta
registros_presence_nat_exot_seca_morta <- registros_corrig %>%
  mutate(
    presence_nativa      = str_detect(!!sym(encostam_vareta_col), "\\bnativa\\b"),
    presence_exotica     = str_detect(!!sym(encostam_vareta_col), "\\bexotica\\b"),
    presence_seca_morta  = str_detect(!!sym(encostam_vareta_col), "\\bseca_morta\\b")
  )

# Summarise presence
encostam_vareta_summary <- registros_presence_nat_exot_seca_morta %>%
  group_by(UC, UA, ANO) %>%
  summarise(
...
# left join presence nativa, exotica, seca_morta
registros_corrig_stat <- registros_corrig_stat %>%
  left_join(encostam_vareta_summary, by = c("UC", "UA", "ANO")) %>%
  relocate(sum_presence_nativa, 
           sum_presence_exotica, 
           sum_presence_seca_morta, .before = sum_herbacea)

#relocate serrapilheira, solo_nu
registros_corrig_stat <- registros_corrig_stat %>%
  relocate(serrapilheira,
           solo_nu, .before = sum_herbacea)
...
rm(registros,
   ...
   sum_form_vida_seca_morta_by_UC_UA_ANO,
   form_veg,
   registros_herb_presence,
   registros_lenh_presence,
   registros_nat_exot_presence_seca_morta,
...
#   
#   plot_p1.1.2_prop_rel_herb_lenh_camp_com_rotulo <- reg_corrig_stat_summarise_p1 %>%
#     subset(., form_veg == "Campestre") %>%
#     ggplot(aes(prop, ANO, fill = categoria)) +
#     geom_col() +
#     geom_text(aes(label = paste0(
...
#   #   plot.title = ggtext::element_textbox_simple())
#   
#   plot_p1.2.2_prop_rel_herb_lenh_sav_com_rotulo <- reg_corrig_stat_summarise_p1 %>%
#     subset(., form_veg == "Savânica") %>%
#     ggplot(aes(prop, ANO, fill = categoria)) +
#     geom_col() +
#     geom_text(aes(label = paste0(
...
# 
# if (
#   sum(select(registros_corrig_stat, matches("sum_presence_herb")), na.rm = TRUE) > 0 |
#   sum(select(registros_corrig_stat, matches("sum_presence_lenh")), na.rm = TRUE) > 0
# ) {
...
#     aes(x = veg_cover, y = factor(ANO), fill = categoria)
#   ) +
...
#       y = "Ano",
#       fill = "Categoria",
#       caption = "Intervalo de confiança de 95% calculado com base na distribuição normal padrão: IC = p ± 1,96 × erro padrão\n(p = proporção de pontos com presença de vegetação na transecção; erro padrão = √[p(1-p)/n], onde n = nº de pontos)"
#     ) +
#     scale_fill_manual(
#       values = c("sum_presence_herb" = "#66c2a5", "sum_presence_lenh" = "#fc8d62"),
#       labels = c("Herbácea", "Lenhosa")
#     ) +
...
#   select(-p, -total_points)
# 
# # Calcular limite do eixo x dinamicamente
# x_max2 <- max(reg_corrig_stat_summarise_p2_presence$veg_cover, na.rm = TRUE) * 1.15
# 
# # Gráfico com rótulos
# plot_p2.3.1_veg_cover_categ_com_rotulo <- ggplot(
#   reg_corrig_stat_summarise_p2_presence %>% 
#     filter(form_veg %in% c("Campestre", "Savânica")),
#   aes(x = veg_cover, y = factor(ANO), fill = categoria)
# ) +
#   geom_col(position = position_dodge(width = 0.7), width = 0.6) +
...
#     y = "Ano",
#     fill = "Categoria",
#     caption = "Intervalo de confiança de 95%: IC = p ± 1,96 × erro padrão\n(p = proporção de pontos com presença; erro padrão = √[p(1-p)/n], n = nº total de pontos)"
#   ) +
#   
...
# 
# 
# plot_p2.1.2_prop_rel_categ_camp_com_rotulo <- reg_corrig_stat_summarise_p2 %>%
#   subset(., form_veg == "Campestre") %>%
#   ggplot(aes(prop, ANO, fill = categoria)) +
#   geom_col() +
#   geom_text(aes(label = paste0(
...
# #   plot.title = ggtext::element_textbox_simple())
# 
# plot_p2.2.2_prop_rel_categ_sav_com_rotulo <- reg_corrig_stat_summarise_p2 %>%
#   subset(., form_veg == "Savânica") %>%
#   ggplot(aes(prop, ANO, fill = categoria)) +
#   geom_col() +
#   geom_text(aes(label = paste0(
...
#   
#     # Limite X dinâmico
#   x_max3 <- max(reg_corrig_stat_summarise_p3_presence$veg_cover, na.rm = TRUE) * 1.15
#   
# # Gráfico com rótulos
#   
...
#   
#   # Limite X dinâmico
#   x_max4 <- max(reg_corrig_stat_summarise_p4_presence$veg_cover, na.rm = TRUE) * 1.15
#   
#   # Gráfico com rótulos
...
#     facet_wrap(~form_veg) +
#     labs(
#       title = "Cobertura de formas de vida de plantas exóticas (Campestre e Savânica)",
#       x = "Cobertura (%)",
#       y = "Ano",
#       fill = "Forma de vida",
#       caption = "Intervalo de confiança de 95%: IC = p ± 1,96 × erro padrão\n(p = proporção de pontos com presença; erro padrão = √[p(1-p)/n], n = nº total de pontos)"
...
#   
#   # Limite X dinâmico
#   x_max5 <- max(reg_corrig_stat_summarise_p5_presence$veg_cover, na.rm = TRUE) * 1.15
#   
#   # Gráfico com rótulos
...
#    registros_corrig_stat_points_fin)
# 
# # Atualizar o cálculo incluindo variação da média
# reg_corrig_stat_summarise_p1 <- registros_corrig_stat %>%
...
#   dplyr::summarise(
#     n = sum(soma, na.rm = TRUE),
#     media = mean(soma, na.rm = TRUE),
#     sd = sd(soma, na.rm = TRUE),
#     cv = ifelse(media != 0, sd/media * 100, 0),  # Coeficiente de Variação (%)
#     n_transectos = n(),
#     .groups = 'drop'
#   ) %>%
...
#   dplyr::mutate(
#     prop = prop.table(n),
#     prop_media = media/sum(media),  # Proporção relativa da média
#     prop_sd = sd/sum(media),        # Desvio padrão da proporção
#     prop_cv = ifelse(prop_media != 0, prop_sd/prop_media * 100, 0)  # CV da proporção
#   ) %>%
#   arrange(ANO, form_veg, categoria)
# 
# # Exportar CSV com novas métricas
# if (exists("reg_corrig_stat_summarise_p1")) {
#   fwrite(
#     reg_corrig_stat_summarise_p1,
#     file.path("prop_rel_herb_lenh_com_variação.csv"),
#     row.names = FALSE
#   )
# }

### distribuicao das medias 21set25:

### Análises Completas com Distribuição das Médias
### Script completo atualizado - Gráficos originais + distribuição das médias com IC
...
### Função para calcular proporções com intervalo de confiança
calcular_proporcao_com_ic <- function(dados, colunas_proporcao, grupo_base = c("ANO", "form_veg")) {
  dados %>%
    dplyr::select(any_of(c("UC", grupo_base, colunas_proporcao))) %>%
    pivot_longer(
      names_to = "categoria",
      values_to = "soma",
      cols = all_of(colunas_proporcao)
    ) %>%
    group_by(across(all_of(c(grupo_base, "categoria")))) %>%
    dplyr::summarise(
      n = sum(soma, na.rm = TRUE),
      n_transectos = n(),
      media = mean(soma, na.rm = TRUE),
    ...
    group_by(across(all_of(grupo_base))) %>%
    dplyr::mutate(
      prop = prop.table(n),
      # Calcular intervalo de confiança para a proporção média
      prop_media = media/sum(media),
      prop_se = sd/sum(media)/sqrt(n_transectos),
      ic_inferior = pmax(0, prop_media - 1.96 * prop_se),
      ic_superior = pmin(1, prop_media + 1.96 * prop_se)
    ) %>%
    arrange(across(all_of(grupo_base)), categoria)
}

### Lista para armazenar todos os plots
todos_os_plots <- list()

cat("Iniciando análises de proporção com distribuição das médias...\n")

### 1. PROPORÇÃO RELATIVA DE PLANTAS HERBÁCEAS E LENHOSAS
cat("\n1. Analisando proporção herbáceas vs lenhosas...\n")
...
sum(select(.data = registros_corrig_stat, which((
  ...
  reg_corrig_stat_summarise_p1 <- calcular_proporcao_com_ic(
    registros_corrig_stat, 
    c("sum_herbacea", "sum_lenhosa")
  )
  
  # Plot Campestre sem rótulo - ORIGINAL
  plot_p1.1.1_prop_rel_herb_lenh_camp_sem_rotulo <- reg_corrig_stat_summarise_p1 %>%
    subset(., form_veg == "Campestre") %>%
    ggplot(aes(prop, ANO, fill = categoria)) +
    geom_col() +
  ...
  print(plot_p1.1.1_prop_rel_herb_lenh_camp_sem_rotulo)
  todos_os_plots[["p1.1.1_herb_lenh_camp_sem_rotulo"]] <- plot_p1.1.1_prop_rel_herb_lenh_camp_sem_rotulo
  
  # Plot Campestre com rótulo - ORIGINAL
  plot_p1.1.2_prop_rel_herb_lenh_camp_com_rotulo <- reg_corrig_stat_summarise_p1 %>%
    subset(., form_veg == "Campestre") %>%
    ggplot(aes(prop, ANO, fill = categoria)) +
    geom_col() +
  ...
  print(plot_p1.1.2_prop_rel_herb_lenh_camp_com_rotulo)
  todos_os_plots[["p1.1.2_herb_lenh_camp_com_rotulo"]] <- plot_p1.1.2_prop_rel_herb_lenh_camp_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Campestre
  plot_p1.1.3_distrib_medias_herb_lenh_camp <- reg_corrig_stat_summarise_p1 %>%
    subset(., form_veg == "Campestre") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.6), size = 3) +
    geom_errorbarh(
      aes(xmin = ic_inferior, xmax = ic_superior),
      height = 0.2, size = 0.8, alpha = 0.7,
      position = position_dodge(width = 0.6)
    ) +
    geom_text(
      aes(label = paste0(scales::percent(prop_media, accuracy = 0.1))),
      position = position_dodge(width = 0.6),
      vjust = -1.2, size = 3, color = "black"
    ) +
    labs(
      title = "Distribuição das médias de proporção - Herbáceas vs Lenhosas\nFormações Campestres",
      x = "Proporção média com IC 95%",
      y = "Ano",
      color = "Categoria",
      caption = "Pontos representam a proporção média, barras horizontais representam intervalos de confiança de 95%"
    ) +
    scale_x_continuous(labels = percent) +
    theme_minimal() +
    theme(legend.position = "bottom")
  
  print(plot_p1.1.3_distrib_medias_herb_lenh_camp)
  todos_os_plots[["p1.1.3_distrib_medias_herb_lenh_camp"]] <- plot_p1.1.3_distrib_medias_herb_lenh_camp
  
  # Plot Savânica sem rótulo - ORIGINAL
  plot_p1.2.1_prop_rel_herb_lenh_sav_sem_rotulo <- reg_corrig_stat_summarise_p1 %>%
    subset(., form_veg == "Savânica") %>%
    ggplot(aes(prop, ANO, fill = categoria)) +
    geom_col() +
  ...
  print(plot_p1.2.1_prop_rel_herb_lenh_sav_sem_rotulo)
  todos_os_plots[["p1.2.1_herb_lenh_sav_sem_rotulo"]] <- plot_p1.2.1_prop_rel_herb_lenh_sav_sem_rotulo
  
  # Plot Savânica com rótulo - ORIGINAL
  plot_p1.2.2_prop_rel_herb_lenh_sav_com_rotulo <- reg_corrig_stat_summarise_p1 %>%
    subset(., form_veg == "Savânica") %>%
    ggplot(aes(prop, ANO, fill = categoria)) +
    geom_col() +
  ...
  print(plot_p1.2.2_prop_rel_herb_lenh_sav_com_rotulo)
  todos_os_plots[["p1.2.2_herb_lenh_sav_com_rotulo"]] <- plot_p1.2.2_prop_rel_herb_lenh_sav_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Savânica
  plot_p1.2.3_distrib_medias_herb_lenh_sav <- reg_corrig_stat_summarise_p1 %>%
    subset(., form_veg == "Savânica") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.6), size = 3) +
    geom_errorbarh(
  ...
  print(plot_p1.2.3_distrib_medias_herb_lenh_sav)
  todos_os_plots[["p1.2.3_distrib_medias_herb_lenh_sav"]] <- plot_p1.2.3_distrib_medias_herb_lenh_sav
}

### 2. ANÁLISE DE PRESENÇA HERBÁCEA/LENHOSA
cat("2. Analisando presença herbácea/lenhosa...\n")

if (sum(select(registros_corrig_stat, matches("sum_presence_herb")), na.rm = TRUE) > 0 |
    sum(select(registros_corrig_stat, matches("sum_presence_lenh")), na.rm = TRUE) > 0) {
  ...
  print(plot_p1.3.1_veg_cover_herb_lenh_com_rotulo)
  todos_os_plots[["p1.3.1_veg_cover_herb_lenh_com_rotulo"]] <- plot_p1.3.1_veg_cover_herb_lenh_com_rotulo
  ...
  print(plot_p1.3.2_veg_cover_herb_lenh_sem_rotulo)
  todos_os_plots[["p1.3.2_veg_cover_herb_lenh_sem_rotulo"]] <- plot_p1.3.2_veg_cover_herb_lenh_sem_rotulo
  
  rm(x_max)
}

### 3. PROPORÇÃO RELATIVA DE CATEGORIAS GERAIS
cat("3. Analisando categorias gerais...\n")

reg_corrig_stat_summarise_p2 <- calcular_proporcao_com_ic(
  registros_corrig_stat,
  c("sum_nativa", "sum_exotica", "sum_seca_morta", "serrapilheira", "solo_nu")
)
...
print(plot_p2.1.1_prop_rel_categ_camp_sem_rotulo)
todos_os_plots[["p2.1.1_categ_camp_sem_rotulo"]] <- plot_p2.1.1_prop_rel_categ_camp_sem_rotulo
...
print(plot_p2.1.2_prop_rel_categ_camp_com_rotulo)
todos_os_plots[["p2.1.2_categ_camp_com_rotulo"]] <- plot_p2.1.2_prop_rel_categ_camp_com_rotulo

# NOVO: Gráfico de pontos com IC para Campestre
plot_p2.1.3_distrib_medias_categ_camp <- reg_corrig_stat_summarise_p2 %>%
  subset(., form_veg == "Campestre") %>%
  ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
  ...
  labs(
    title = "Distribuição das médias de proporção - Categorias Gerais\nFormações Campestres",
    x = "Proporção média com IC 95%",
    y = "Ano",
    color = "Categoria",
    caption = "Pontos representam a proporção média, barras horizontais representam intervalos de confiança de 95%"
...
print(plot_p2.1.3_distrib_medias_categ_camp)
todos_os_plots[["p2.1.3_distrib_medias_categ_camp"]] <- plot_p2.1.3_distrib_medias_categ_camp
...
print(plot_p2.2.1_prop_rel_categ_sav_sem_rotulo)
todos_os_plots[["p2.2.1_categ_sav_sem_rotulo"]] <- plot_p2.2.1_prop_rel_categ_sav_sem_rotulo
...
print(plot_p2.2.2_prop_rel_categ_sav_com_rotulo)
todos_os_plots[["p2.2.2_categ_sav_com_rotulo"]] <- plot_p2.2.2_prop_rel_categ_sav_com_rotulo

# NOVO: Gráfico de pontos com IC para Savânica
plot_p2.2.3_distrib_medias_categ_sav <- reg_corrig_stat_summarise_p2 %>%
  subset(., form_veg == "Savânica") %>%
  ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
  geom_point(position = position_dodge(width = 0.7), size = 3) +
  geom_errorbarh(
...
print(plot_p2.2.3_distrib_medias_categ_sav)
todos_os_plots[["p2.2.3_distrib_medias_categ_sav"]] <- plot_p2.2.3_distrib_medias_categ_sav

### 4. PRESENÇA CATEGORIAS GERAIS
cat("4. Analisando presença categorias gerais...\n")

reg_corrig_stat_summarise_p2_presence <- registros_corrig_stat %>%
  select(any_of(c("UC", "UA", "ANO", "form_veg", 
                  "sum_presence_nativa", "sum_presence_exotica", 
                  "sum_presence_seca_morta", "serrapilheira", "solo_nu"))) %>%
  pivot_longer(
    names_to = "categoria",
    values_to = "soma",
    cols = c("sum_presence_nativa", "sum_presence_exotica", 
             "sum_presence_seca_morta", "serrapilheira", "solo_nu")
  ) %>%
  group_by(ANO, form_veg, categoria) %>%
  summarise(
...
print(plot_p2.3.1_veg_cover_categ_com_rotulo)
todos_os_plots[["p2.3.1_veg_cover_categ_com_rotulo"]] <- plot_p2.3.1_veg_cover_categ_com_rotulo
...
print(plot_p2.3.2_veg_cover_categ_sem_rotulo)
todos_os_plots[["p2.3.2_veg_cover_categ_sem_rotulo"]] <- plot_p2.3.2_veg_cover_categ_sem_rotulo

rm(x_max2)

### 5. FORMAS DE VIDA NATIVAS
cat("5. Analisando formas de vida nativas...\n")

if (sum(select(.data = registros_corrig_stat, which((
  str_detect(colnames(registros_corrig_stat), "nativa_", negate = FALSE)
)))) > 0) {
  
  colunas_nativas <- colnames(registros_corrig_stat)[str_detect(colnames(registros_corrig_stat), "nativa_")]
  reg_corrig_stat_summarise_p3 <- calcular_proporcao_com_ic(
    registros_corrig_stat,
    colunas_nativas
  )
  ...
  print(plot_p3.1.1_prop_rel_nat_camp_sem_rotulo)
  todos_os_plots[["p3.1.1_nat_camp_sem_rotulo"]] <- plot_p3.1.1_prop_rel_nat_camp_sem_rotulo
  ...
  print(plot_p3.1.2_prop_rel_nat_camp_com_rotulo)
  todos_os_plots[["p3.1.2_nat_camp_com_rotulo"]] <- plot_p3.1.2_prop_rel_nat_camp_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Campestre
  plot_p3.1.3_distrib_medias_nat_camp <- reg_corrig_stat_summarise_p3 %>%
    subset(., form_veg == "Campestre") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.7), size = 3) +
    geom_errorbarh(
    ...
    labs(
      title = "Distribuição das médias de proporção - Formas de Vida Nativas\nFormações Campestres",
      x = "Proporção média com IC 95%",
      y = "Ano",
      color = "Forma de Vida",
      caption = "Pontos representam a proporção média, barras horizontais representam intervalos de confiança de 95%"
  ...
  print(plot_p3.1.3_distrib_medias_nat_camp)
  todos_os_plots[["p3.1.3_distrib_medias_nat_camp"]] <- plot_p3.1.3_distrib_medias_nat_camp
  ...
  print(plot_p3.2.1_prop_rel_nat_sav_sem_rotulo)
  todos_os_plots[["p3.2.1_nat_sav_sem_rotulo"]] <- plot_p3.2.1_prop_rel_nat_sav_sem_rotulo
  ...
  print(plot_p3.2.2_prop_rel_nat_sav_com_rotulo)
  todos_os_plots[["p3.2.2_nat_sav_com_rotulo"]] <- plot_p3.2.2_prop_rel_nat_sav_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Savânica
  plot_p3.2.3_distrib_medias_nat_sav <- reg_corrig_stat_summarise_p3 %>%
    subset(., form_veg == "Savânica") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.7), size = 3) +
    geom_errorbarh(
  ...
  print(plot_p3.2.3_distrib_medias_nat_sav)
  todos_os_plots[["p3.2.3_distrib_medias_nat_sav"]] <- plot_p3.2.3_distrib_medias_nat_sav
}

### 6. FORMAS DE VIDA EXÓTICAS
cat("6. Analisando formas de vida exóticas...\n")

if (sum(select(.data = registros_corrig_stat, which((
  str_detect(colnames(registros_corrig_stat), "exot_", negate = FALSE)
)))) > 0) {
  
  colunas_exoticas <- colnames(registros_corrig_stat)[str_detect(colnames(registros_corrig_stat), "exot_")]
  reg_corrig_stat_summarise_p4 <- calcular_proporcao_com_ic(
    registros_corrig_stat,
    colunas_exoticas
  )
  ...
  print(plot_p4.1.1_prop_rel_exot_camp_sem_rotulo)
  todos_os_plots[["p4.1.1_exot_camp_sem_rotulo"]] <- plot_p4.1.1_prop_rel_exot_camp_sem_rotulo
  ...
  print(plot_p4.1.2_prop_rel_exot_camp_com_rotulo)
  todos_os_plots[["p4.1.2_exot_camp_com_rotulo"]] <- plot_p4.1.2_prop_rel_exot_camp_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Campestre
  plot_p4.1.3_distrib_medias_exot_camp <- reg_corrig_stat_summarise_p4 %>%
    subset(., form_veg == "Campestre") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.7), size = 3) +
    geom_errorbarh(
    ...
    labs(
      title = "Distribuição das médias de proporção - Formas de Vida Exóticas\nFormações Campestres",
      x = "Proporção média com IC 95%",
      y = "Ano",
      color = "Forma de Vida",
      caption = "Pontos representam a proporção média, barras horizontais representam intervalos de confiança de 95%"
  ...
  print(plot_p4.1.3_distrib_medias_exot_camp)
  todos_os_plots[["p4.1.3_distrib_medias_exot_camp"]] <- plot_p4.1.3_distrib_medias_exot_camp
  ...
  print(plot_p4.2.1_prop_rel_exot_sav_sem_rotulo)
  todos_os_plots[["p4.2.1_exot_sav_sem_rotulo"]] <- plot_p4.2.1_prop_rel_exot_sav_sem_rotulo
  ...
  print(plot_p4.2.2_prop_rel_exot_sav_com_rotulo)
  todos_os_plots[["p4.2.2_exot_sav_com_rotulo"]] <- plot_p4.2.2_prop_rel_exot_sav_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Savânica
  plot_p4.2.3_distrib_medias_exot_sav <- reg_corrig_stat_summarise_p4 %>%
    subset(., form_veg == "Savânica") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.7), size = 3) +
    geom_errorbarh(
  ...
  print(plot_p4.2.3_distrib_medias_exot_sav)
  todos_os_plots[["p4.2.3_distrib_medias_exot_sav"]] <- plot_p4.2.3_distrib_medias_exot_sav
}

### 7. FORMAS DE VIDA SECAS/MORTAS
cat("7. Analisando formas de vida secas/mortas...\n")

if (sum(select(.data = registros_corrig_stat, which((
  str_detect(colnames(registros_corrig_stat), "seca_morta_", negate = FALSE)
)))) > 0) {
  
  colunas_seca_morta <- colnames(registros_corrig_stat)[str_detect(colnames(registros_corrig_stat), "seca_morta_")]
  reg_corrig_stat_summarise_p5 <- calcular_proporcao_com_ic(
    registros_corrig_stat,
    colunas_seca_morta
  )
  ...
  plot_p5.1.1_prop_rel_seca_morta_camp_sem_rotulo <- reg_corrig_stat_summarise_p5 %>%
    ...
    geom_col() +
    labs(
      title = "Proporção relativa de formas de vida e outras categorias de\nplantas secas ou mortas em formações campestres",
      x = "Proporção relativa",
      y = "ANO",
      fill = "Categoria"
    ) +
  ...
  print(plot_p5.1.1_prop_rel_seca_morta_camp_sem_rotulo)
  todos_os_plots[["p5.1.1_seca_morta_camp_sem_rotulo"]] <- plot_p5.1.1_prop_rel_seca_morta_camp_sem_rotulo
  ...
  print(plot_p5.1.2_prop_rel_seca_morta_camp_com_rotulo)
  todos_os_plots[["p5.1.2_seca_morta_camp_com_rotulo"]] <- plot_p5.1.2_prop_rel_seca_morta_camp_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Campestre
  plot_p5.1.3_distrib_medias_seca_morta_camp <- reg_corrig_stat_summarise_p5 %>%
    subset(., form_veg == "Campestre") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.7), size = 3) +
    geom_errorbarh(
    ...
    labs(
      title = "Distribuição das médias de proporção - Formas de Vida Secas/Mortas\nFormações Campestres",
      x = "Proporção média com IC 95%",
      y = "Ano",
      color = "Forma de Vida",
      caption = "Pontos representam a proporção média, barras horizontais representam intervalos de confiança de 95%"
  ...
  print(plot_p5.1.3_distrib_medias_seca_morta_camp)
  todos_os_plots[["p5.1.3_distrib_medias_seca_morta_camp"]] <- plot_p5.1.3_distrib_medias_seca_morta_camp
  ...
  plot_p5.2.1_prop_rel_seca_morta_sav_sem_rotulo <- reg_corrig_stat_summarise_p5 %>%
    ...
    geom_col() +
    labs(
      title = "Proporção relativa de formas de vida e outras categorias de\nplantas secas ou mortas em formações savânicas",
      x = "Proporção relativa",
      y = "ANO",
      fill = "Categoria"
    ) +
  ...
  print(plot_p5.2.1_prop_rel_seca_morta_sav_sem_rotulo)
  todos_os_plots[["p5.2.1_seca_morta_sav_sem_rotulo"]] <- plot_p5.2.1_prop_rel_seca_morta_sav_sem_rotulo
  ...
  print(plot_p5.2.2_prop_rel_seca_morta_sav_com_rotulo)
  todos_os_plots[["p5.2.2_seca_morta_sav_com_rotulo"]] <- plot_p5.2.2_prop_rel_seca_morta_sav_com_rotulo
  
  # NOVO: Gráfico de pontos com IC para Savânica
  plot_p5.2.3_distrib_medias_seca_morta_sav <- reg_corrig_stat_summarise_p5 %>%
    subset(., form_veg == "Savânica") %>%
    ggplot(aes(x = prop_media, y = factor(ANO), color = categoria)) +
    geom_point(position = position_dodge(width = 0.7), size = 3) +
    geom_errorbarh(
  ...
  print(plot_p5.2.3_distrib_medias_seca_morta_sav)
  todos_os_plots[["p5.2.3_distrib_medias_seca_morta_sav"]] <- plot_p5.2.3_distrib_medias_seca_morta_sav
}

### EXPORTAÇÃO DE ARQUIVOS
cat("\n8. Exportando arquivos...\n")

dir.create("plots_png", showWarnings = FALSE)

walk2(
  .x = todos_os_plots,
  .y = names(todos_os_plots),
  .f = ~{
    tryCatch({
      ggsave(
        filename = file.path("plots_png", paste0(.y, ".png")),
      ...
      cat("Erro ao salvar plot", .y, ":", e$message, "\n")
    })
  }
)
...
# Exportar CSVs
if (exists("registros_corrig")) {
  fwrite(registros_corrig, "registros_corrig.csv", row.names = FALSE)
  cat("CSV salvo: registros_corrig.csv\n")
}

if (exists("registros_corrig_stat")) {
  fwrite(registros_corrig_stat, "registros_corrig_stat.csv", row.names = FALSE)
  cat("CSV salvo: registros_corrig_stat.csv\n")
}

# Exportar datasets com IC
if (exists("reg_corrig_stat_summarise_p1")) {
  fwrite(reg_corrig_stat_summarise_p1, "prop_rel_herb_lenh_com_ic.csv", row.names = FALSE)
  cat("CSV salvo: prop_rel_herb_lenh_com_ic.csv\n")
}

if (exists("reg_corrig_stat_summarise_p2")) {
  fwrite(reg_corrig_stat_summarise_p2, "prop_rel_categ_com_ic.csv", row.names = FALSE)
  cat("CSV salvo: prop_rel_categ_com_ic.csv\n")
}

if (exists("reg_corrig_stat_summarise_p3")) {
  fwrite(reg_corrig_stat_summarise_p3, "prop_rel_form_vida_nat_com_ic.csv", row.names = FALSE)
  cat("CSV salvo: prop_rel_form_vida_nat_com_ic.csv\n")
}

if (exists("reg_corrig_stat_summarise_p4")) {
  fwrite(reg_corrig_stat_summarise_p4, "prop_rel_form_vida_exot_com_ic.csv", row.names = FALSE)
  cat("CSV salvo: prop_rel_form_vida_exot_com_ic.csv\n")
}

if (exists("reg_corrig_stat_summarise_p5")) {
  fwrite(reg_corrig_stat_summarise_p5, "prop_rel_form_vida_seca_morta_com_ic.csv", row.names = FALSE)
  cat("CSV salvo: prop_rel_form_vida_seca_morta_com_ic.csv\n")
}

if (exists("todos_os_plots")) {
  rm(todos_os_plots)
}

if (exists("colunas_exoticas")) {
  rm(colunas_exoticas)
}

if (exists("colunas_nativas")) {
  rm(colunas_nativas)
}

if (exists("colunas_seca_morta")) {
  rm(colunas_seca_morta)
}

if (exists("calcular_proporcao_com_ic")) {
  rm(calcular_proporcao_com_ic)
}
