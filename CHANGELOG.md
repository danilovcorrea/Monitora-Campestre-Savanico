...
- Melhora mensagens, progresso e rastreabilidade dos avisos.
- Encurta caminhos para OneDrive preservando identidade, dados e linhagem.
- Confere entregas EET e PNM e registra pendências operacionais nas notas.
- Mantém o script autônomo abaixo de 5 MB inclusive CRLF.
...
- Evidências temporais principais passam a usar todas as UAs comuns a cada par
  de anos; painéis por entrada permanecem como sensibilidade.
- A seção de época recupera a tabela compacta de campanhas e explicita os
  recortes dos cenários de referência antes da recomendação de linha de base.
- Formas de vida sem qualquer toque na UC deixam de ocupar as tabelas do PDF,
  sem remoção das bases tabulares auditáveis.

### Projeto QField

- `qfield_input/` aceita, em rodada de uma UC, MBTiles com nome arbitrário e
  camadas vetoriais sem exigir subpasta específica da UC.
- A classificação da imagem usa o zoom real; o recorte circular de 500 m é
  derivado dos consensos espaciais e executado internamente, com fonte intacta.
- Auditorias registram cobertura, hashes, tiles, bordas, descartes e redução.

### Manual e preservação

- O manual gerado documenta as rotinas integrais, configurações, saídas,
  relatórios, QField, continuidade e diagnóstico para usuários não treinados.
- Homologações PNB, APA Ibirapuitã, Linux e R 4.6.0 do Windows aprovadas.
- Inicialização do RStudio, contrato XLSForm e dados biológicos preservados.
...
- Relatório de validação: sessões atuais e herdadas, efeitos comprovados e não
  quantificáveis, histórico anual, exclusões, equipe e validação espacial.
- Relatório analítico: precisão para categorias raras, distinção do instrumento
  botânico anterior a 2025, interpretação de estabilidade e análise complementar
  da época das campanhas com recomendação condicional de linha de base.

### Navegação QField

- Projeto offline opcional com Sentinel regional, MBTiles detalhado fornecido,
  camadas por UA/ano e incorporações espaciais declaradas pelo usuário.
- Harmonização das posições de navegação com o validador espacial central;
  cobertura detalhada e provenance auditadas antes de empacotar.

### Preservação

- Contrato XLSForm, inicialização RStudio e fluxo anterior preservados; gates
  Linux/Windows e rodada real PNM aprovados. Script abaixo de 5 MB em CRLF.

## v2.9.24 - 2026-09-09

### Entradas e painel

- Exportações XLSX com abas adicionais passam a localizar a aba biológica pelo
  esquema esperado, preservando texto e codificação sem conversão silenciosa.
- Representações físicas concorrentes do mesmo campo são conciliadas somente
  quando não há divergência; o preflight atômico antecede a persistência da
  intenção no painel.
- O leitor incremental testa a preservação de aspas pelo `data.table::fread` e
  usa `utils::read.csv` apenas quando o runtime afetado falha nesse teste.
...
- Relatórios analíticos explicitam UAs, registros e pontos com presença,
  padronizam cobertura e proporção relativa e tratam corretamente campanhas
  únicas, textos condicionais e evidências de solo/rochas.
- Grafias históricas equivalentes da mesma UC são reconciliadas apenas nas
  camadas derivadas de estatísticas e relatórios, sem regravar dados ou
  linhagem.

### Homologação e preservação

- Vinte e dois produtos de recuperação foram aprovados por auditoria externa,
  com mapas Sentinel exigidos materializados antes da cópia para o OneDrive.
- Gates Linux e Windows R 4.6.0 aprovaram o arquivo público, inclusive o caso
  real com 2.048 aspas por valor, o limite CRLF, o contrato integral e o início
  congelado do RStudio.
- Contrato único, XLSForm 2025 e módulos não alcançados permanecem inalterados.
