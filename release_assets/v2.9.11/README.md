...
- Versão: `v2.9.11`
- Script principal: [`monitora_campsav_alvo_global_v2.9.11.R`](monitora_campsav_alvo_global_v2.9.11.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.11/`](releases/v2.9.11/)
- Notas da versão: [`RELEASE_NOTES_v2.9.11.md`](RELEASE_NOTES_v2.9.11.md)
- Release no GitHub: [v2.9.11](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.11)
...
## Destaques da v2.9.11

- Relatórios com uma única campanha passam a apresentar uma linha de base
  transversal entre UAs, sem classificar aumento, redução, estabilidade ou
  tendência temporal sem suporte amostral.
- O mapa Sentinel-2 amplia progressivamente a janela de busca quando a imagem
  mais recente não atende aos critérios de cobertura e qualidade. A janela,
  a data da aquisição, a defasagem e a cobertura são informadas no mapa e na
  auditoria.
- O localizador obtém rede, UC, estados e biomas por consultas independentes,
  auditáveis e resilientes. Uma falha parcial preserva as camadas disponíveis.
- A persistência de justificativas e checkpoints usa publicação transacional,
  validação por hash, retentativas curtas para bloqueios transitórios e rollback
  integral em falhas.
- A prévia integral reutiliza o contrato de edição calculado uma única vez,
  reduzindo o custo de filas volumosas sem alterar regras ou resultados.
- O rótulo legado `amostragem/especie` volta a exibir a pergunta do XLSForm em
  linguagem legível, mantendo o nome técnico para rastreabilidade.

### Preservado da v2.9.10
...
Os padrões da v2.9.11 representam a continuidade incremental integral usada pela
...
2. Copie `monitora_campsav_alvo_global_v2.9.11.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.11.R`.
...
A v2.9.11 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.11.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.11.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.11.md`: roteiro operacional resumido.
- `release_assets/v2.9.11/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.11/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.11. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.11. Acesso em: DD mês AAAA.
```
