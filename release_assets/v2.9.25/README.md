...
- Versão: `v2.9.25`
- Script principal: [`monitora_campsav_alvo_global_v2.9.25.R`](monitora_campsav_alvo_global_v2.9.25.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.25/`](releases/v2.9.25/)
- Notas da versão: [`RELEASE_NOTES_v2.9.25.md`](RELEASE_NOTES_v2.9.25.md)
- Release no GitHub: [v2.9.25](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.25)
...
- Exportações XLSX recentes do SISMONITORA localizam a aba biológica pelo
  ...
  silenciosa de texto ou codificação.
- Representações físicas concorrentes do mesmo campo são reconciliadas antes
  do painel somente quando não divergem. A intenção de correção só é
  persistida depois do preflight semântico completo.
- O relatório de validação separa arquivos atuais e herdados, modificações do
  bolsista, modificações automáticas e registros técnicos de auditoria,
  recuperação e conciliação.
- Relatórios analíticos explicitam UAs, registros e pontos com presença,
  padronizam cobertura e proporção relativa e tratam campanhas únicas e textos
  condicionais sem produzir comparações artificiais.
- Grafias históricas equivalentes da mesma UC são reconciliadas apenas nas
  cópias derivadas dos relatórios e estatísticas; dados e linhagem permanecem
  inalterados.
- O leitor incremental testa se `data.table::fread` preserva aspas escapadas e
  usa a base R somente em runtimes afetados. O caso real com 2.048 aspas por
  valor foi aprovado no Linux e no R 4.6.0 do Windows.
- O contrato único, o XLSForm 2025 e a inicialização do RStudio permanecem
  integralmente inalterados.

### Preservado da v2.9.23

- A validação pré-painel reconhece o label exato de uma choice `select_one`
  quando o próprio contrato único estabelece uma relação unívoca com o name.
  A resolução ocorre somente na cópia usada pela validação e não regrava o
  valor original do dataset.
- `select_multiple` continua estrito, sem tradução de labels, alias local,
  normalização por caixa ou acento e sem flexibilização do XLSForm 2025.
- Ocorrências de token fora do domínio na mesma linha são distinguidas pelo
  caminho contratual, pela lista e pelo token inválido. Essa identidade também
  é usada pelas justificativas persistentes, evitando colisões e duplicações.
- Casos reais no PNI e no RVSVOB foram homologados no Linux, no R 4.6.0 e no
  RStudio do Windows, com contagens exatas, zero duplicata semântica e
  desempenho preservado.
- O contrato XLSForm 2025 permanece integralmente inalterado.

### Preservado da v2.9.22

- Um gate pré-painel verifica os valores `select_one` e `select_multiple`
  diretamente contra as `choices` vigentes do contrato único/XLSForm 2025.
  Tokens fora do domínio são bloqueados sem alias ou mapa local concorrente.
- Correções de hábito passam por uma validação fechada de relevância por
  categoria, forma e campo físico antes da aplicação. O ramo errado não pode
  receber a decisão destinada a outro contexto.
- A fila admite `replace_token` seguido de `append_token` na mesma célula
  somente quando origem, destino e token acrescentado são distintos. A
  decomposição continua atômica e sobreposições permanecem conflitantes.
- Identificadores estáveis com os textos literais `NA` ou `NULL` deixam de ser
  aceitos como chaves válidas.
- O contrato XLSForm 2025 permanece integralmente inalterado.

### Preservado da v2.9.21

- Dependências `relevance` somente fecham campos superiores quando o token
  também pertence às `choices` vigentes do pai. O token histórico órfão
  `outros` deixa de ser criado ou aceito como forma de vida exótica.
- Checkpoints legados com `outros` podem ser reconciliados automaticamente
  quando há exatamente uma forma exótica válida já marcada, texto de espécie
  preservável e nenhum conflito no ramo de destino. Ambiguidades continuam
  bloqueadas sem mutação parcial.
- O preenchimento explícito de “outra espécie exótica” materializa somente os
  ancestrais semânticos válidos do XLSForm 2025. Módulo histórico, tokens de
  impactos e “outra forma de vida” permanecem fora dessa regra.
- O contrato XLSForm embutido permanece integralmente idêntico ao da v2.9.20.
  A execução real de 20.705 registros alterou somente 20 células autorizadas
  em quatro registros, manteve a cardinalidade e concluiu com zero bloqueio.

### Preservado da v2.9.20

- A leitura dos COGs Sentinel-2 deixa de reatribuir georreferência a visões
  internas. O GDAL somente utiliza uma visão quando bandas, CRS, extensão e
  resolução permanecem coerentes com o arquivo oficial.
- Caches antigos sem garantia espacial são rejeitados; a rotina retorna ao COG
  principal quando a visão interna não atende ao contrato.
- A exclusão de uma correção pendente é efetivada mesmo que outras operações da
  fila ainda sejam conflitantes. Os conflitos restantes continuam auditados e
  bloqueados até serem resolvidos.
- Testes com COGs oficiais, execução real sem cache e Windows confirmaram a
  correção, sem alteração dos produtos de dados ou regressão de inicialização.

### Preservado da v2.9.19

- A sanitização assistida de hábitos obrigatórios passa a usar a mesma
  elegibilidade contratual no diagnóstico, na prévia, na aplicação e no replay.
- Valores inválidos não vazios, como `?`, são efetivamente substituídos quando
  a ocorrência foi identificada pelo contrato XLSForm. Valores já válidos
  permanecem protegidos contra sobrescrita.
- A operação continua atômica, idempotente e registrada na linhagem. A
  homologação real no Windows confirmou a correção nos produtos corrigido,
  validado e de importação, sem perda do histórico acumulado.
- Inicialização rápida do RStudio, contrato único, painel, demais correções,
  produtos, relatórios, estatísticas, mapas e desempenho permanecem preservados.

### Preservado da v2.9.18

- O arquivo único foi reduzido para 4,47 MB em LF e 4,55 MB após conversão
  integral para CRLF, permanecendo 693.541 bytes abaixo do limite rígido de
  5 MiB do editor do RStudio para Windows.
- O contrato consolidado dos XLSForms permanece embutido no próprio `.R`,
  agora em serialização portável, compactada e validada por componentes,
  cardinalidade e igualdade integral de conteúdo.
- A compactação remove apenas redundância física. Código executável,
  inicialização rápida, dados, painel, linhagem, produtos, análises e mapas
  permanecem semanticamente idênticos à v2.9.17.
- O repositório fixa finais de linha LF para scripts R, impedindo que um clone
  no Windows volte a ultrapassar o limite do RStudio por conversão automática.

### Preservado da v2.9.17

- O relatório consolidado de validação passa a oferecer Markdown e DOCX
  editáveis, com modelo A4 embutido, tabelas legíveis, hyperlinks e publicação
  protegida contra caminhos longos no Word para Windows.
- Os achados analíticos identificam o painel fixo, seu ano inicial e o número
  de UAs comparadas. A direção observada também é confrontada com cada painel
  iniciado após ampliação do esforço, sem misturar populações analíticas.
- O índice de evidências registra o ano inicial do painel e a população
  analítica. A redação de valores de p inferiores a 0,001 foi normalizada sem
  alterar cálculos, denominadores ou critérios estatísticos.
- Um resolvedor físico central liga paths, names, labels e aliases exatos ao
  contrato único. Ambiguidades ou duplicidades falham de modo fechado.
- O fechamento hierárquico de formas de vida usa somente as dependências do
  XLSForm 21FEV25, inclusive relações transitivas; texto livre deixa de ser
  interpretado como token, eliminando falsas ocorrências de “outra forma de
  vida”.
- A triagem de secas ou mortas e os relatórios analíticos compartilham a mesma
  resolução contratual dos atributos de impactos, distinguindo campo vazio,
  atributo ausente e resolução ambígua.
- A resolução é memorizada pela assinatura das colunas e não percorre linhas
  adicionais do conjunto de dados. Os gates reais não apontaram regressão de
  desempenho.

### Preservado da v2.9.16

- A aquisição Sentinel-2 percorre as páginas do catálogo, amplia a janela
...
- A continuidade incremental passa a importar e verificar o inventário de
  sessões quando ele estiver assinado pelo manifesto, preservando inclusive
  execuções legadas sem decisões semânticas.
- A ordem histórica herdada permanece estável, a sessão atual é acrescentada
  ao final e o novo inventário é novamente assinado; nenhuma sessão de
  inventário cria eventos semânticos artificiais.
- A recuperação e a atualização foram homologadas em 12 UCs, com preservação
  integral das linhas e da linhagem, relatórios e cartografia nas execuções
  completas e continuidade controlada nas bases com pendências impeditivas.

### Preservado das versões anteriores

- A incorporação recursiva de novas COLETAs e a edição contratual de UA
  publicadas na v2.9.14 permanecem atômicas e inalteradas.
- Relatórios analíticos continuam separando série inicial, UAs comuns pareadas
  e painéis iniciados em cada ampliação do esforço, dentro da seção temática.
- Figuras técnicas e dos relatórios conservam paleta semântica, iconografia e
  disposição estatística unificadas.
- A seleção Sentinel-2, a síntese de fogo, a persistência dos hábitos e as
  hipóteses ecológicas condicionadas às evidências permanecem inalteradas.
...
Os padrões da v2.9.16 representam a continuidade incremental integral usada pela
...
Na v2.9.16, o manifesto passa a assinar também
`inventario_sessoes_linhagem.csv`. Copie sempre a pasta `linhagem/` inteira da
execução anterior: a ordem das sessões herdadas é preservada e a nova sessão
é anexada ao final, mesmo quando uma execução legada não possui data ou
decisões semânticas registradas.
...
2. Copie `monitora_campsav_alvo_global_v2.9.18.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.18.R`.
...
A v2.9.18 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.25.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.25.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.25.md`: roteiro operacional resumido.
- `release_assets/v2.9.25/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.25/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.25. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.25. Acesso em: DD mês AAAA.
```
