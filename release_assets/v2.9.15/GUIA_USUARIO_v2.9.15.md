# Guia do usuário — v2.9.15

## Preparação

1. Crie uma pasta de execução vazia.
2. Copie `monitora_campsav_alvo_global_v2.9.15.R` para essa pasta.
...
**Source** normalmente. A v2.9.15 preserva a arquitetura homologada na v2.9.1:
...
## Novidades operacionais da v2.9.15

- Relatórios analíticos não são interrompidos quando um recorte por formação
  vegetacional fica vazio; tabelas e figuras disponíveis continuam sendo
  materializadas com o schema esperado.
- A cartografia do localizador usa estados e biomas das edições oficiais mais
  recentes do IBGE e UCs federais do ICMBio/INDE. A auditoria registra a fonte,
  a edição, a escala, a atualização, o checksum e eventual fallback.
- Se duas ações tentarem alterar a mesma ocorrência `desconhecida`, o painel
  reconhece o mesmo alvo semântico e exige a revisão do conflito antes da
  prévia; nenhuma alteração parcial é aplicada.
- Correções históricas que acrescentam texto podem ser reaplicadas de forma
  idempotente. O fragmento já presente não é duplicado.
- O mapa Sentinel e as demais figuras esperadas no DOCX são conferidos dentro
  do próprio arquivo Word. Um documento sem imagem, legenda ou relacionamento
  válido não é aceito como relatório concluído.
- Os painéis estatísticos completos disponíveis integram o relatório detalhado
  junto ao respectivo tema. O sintético permanece executivo e os testes não
  são recalculados durante a incorporação.
- A capa Word adota os mesmos elementos institucionais e informativos da capa
  PDF/HTML; as páginas seguintes retomam cabeçalho, rodapé e numeração.
- A incorporação recursiva de novas COLETAs e a edição de UA entre
  `UA-001_VgCS` e `UA-999_VgCS` permanecem disponíveis conforme a v2.9.14.
- O relatório detalhado organiza cada indicador pela série inicial e pelos
  painéis que começam em anos de ampliação do esforço. As UAs comuns pareadas,
  todas as UAs observadas e a composição do painel permanecem leituras
  distintas.
- Gráficos técnicos e figuras dos relatórios usam a mesma paleta semântica e a
  mesma iconografia estatística. Eixos, rótulos externos, linhas de ligação e
  áreas de anotação se ajustam ao conteúdo.
- A seção de hipóteses ecológicas só exibe situações acionadas por evidências
  da UC. O texto separa achado, hipótese compatível e evidência necessária.
- A tabela de impactos apresenta percentuais anuais e um contexto agregado de
  fogo por COLETAs únicas, acompanhado dos subcontextos declarados.
- O mapa Sentinel seleciona aquisições pela menor nebulosidade local, com
  desempate pela data mais recente e ampliação progressiva limitada da busca.
- Operações assistidas de hábito persistem no campo físico correto da forma de
  vida, inclusive para valores terrestre, epífita e rupícola em estruturas
  históricas.
- Arquivos brutos com COLETAs omitidas podem ser incorporados de forma
  transacional em uma continuidade incremental mediante opção explícita.
- As dependências dos produtos opcionais estão incorporadas ao próprio script
  e são verificadas somente quando o módulo é ativado.
...
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS <- "S"
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS_IMPORTACAO_SISMONITORA <- "S"
MONITORA_OPCAO_REMOVER_UUID_IMPORTACAO_SISMONITORA <- "N" # padrão v2.9.15
```
...
Em lotes de atributos superiores, a v2.9.15 usa o valor bruto exato de cada
