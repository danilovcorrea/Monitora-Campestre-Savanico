...
- Versão: `v2.7.4`
- Script principal: [`monitora_campsav_alvo_global_v2.7.4.R`](monitora_campsav_alvo_global_v2.7.4.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.7.4/`](releases/v2.7.4/)
- Notas da versão: [`RELEASE_NOTES_v2.7.4.md`](RELEASE_NOTES_v2.7.4.md)
- Release no GitHub: [v2.7.4](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.7.4)
...
- Replay semântico documentado como reconstrução a partir de uma cópia idêntica do input bruto e do ledger consolidado, separado da continuidade incremental.
- Configuração fail-closed: replay sem ledger, em modo incompatível ou combinado indevidamente com oráculo é bloqueado antes do processamento.
- Gate opcional e estrito de convergência entre o `registros_corrig` final reconciliado e uma run-oráculo, executado antes da exportação.
- Comparação por identidade canônica única, com reconciliação unívoca de aliases de colunas e relatórios detalhados de chaves, colunas e células divergentes.
- Preservação integral do painel, da fila, dos atributos editáveis, das operações semânticas, dos resolvedores e dos 13 modos públicos.
- Replay validado com movimentos, SANHAB e exclusão de coletas, incluindo testes positivos e bloqueios deliberados por divergência.
...
2. Copie `monitora_campsav_alvo_global_v2.7.4.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.7.4.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.7.4 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.7.4.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.7.4.md`: notas da versão atual.
- `GUIA_USUARIO_v2.7.4.md`: roteiro operacional resumido.
- `release_assets/v2.7.4/`: conjunto mínimo de artefatos da release.
- `releases/v2.7.4/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.7.4. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.7.4. Acesso em: DD mês AAAA.
```
