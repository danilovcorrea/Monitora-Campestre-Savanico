...
- Versão: `v2.8.2`
- Script principal: [`monitora_campsav_alvo_global_v2.8.2.R`](monitora_campsav_alvo_global_v2.8.2.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.8.2/`](releases/v2.8.2/)
- Notas da versão: [`RELEASE_NOTES_v2.8.2.md`](RELEASE_NOTES_v2.8.2.md)
- Release no GitHub: [v2.8.2](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.8.2)
...
- Novo produto opcional `registros_validados_importacao_sismonitora.xlsx`,
  derivado exclusivamente de `registros_validados.csv` gerado e aprovado na
  mesma execução.
- Planilha destinada à inclusão de registros novos provenientes de formulários
  em papel, com as três abas do modelo 21FEV25 e a coluna `uc` explícita.
- As colunas `uuid` e `amostragem/registro/uuid` são preservadas no schema, mas
  permanecem vazias para que o SISMONITORA atribua novas identidades.
- Os UUIDs existentes na fonte permanecem intocados em
  `registros_validados.csv` e são rastreados na auditoria; a planilha não deve
  ser usada para editar registros já cadastrados.
- Campos comuns são gravados somente na primeira linha de cada bloco de 101
  pontos, conforme o contrato de importação.
- O produto permanece desligado por padrão e seu módulo não é materializado,
  não lê dados nem carrega dependências adicionais quando a opção está em `N`.
- Interface, painel, 13 modos públicos, contrato único, produtos centrais,
  replay, validação espacial, KML/KMZ e correções publicadas na v2.8.1
...
2. Copie `monitora_campsav_alvo_global_v2.8.2.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.8.2.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.8.2 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.8.2.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.8.2.md`: notas da versão atual.
- `GUIA_USUARIO_v2.8.2.md`: roteiro operacional resumido.
- `release_assets/v2.8.2/`: conjunto mínimo de artefatos da release.
- `releases/v2.8.2/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.8.2. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.8.2. Acesso em: DD mês AAAA.
```
