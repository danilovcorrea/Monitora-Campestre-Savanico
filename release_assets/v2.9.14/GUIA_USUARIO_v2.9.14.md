# Guia do usuário — v2.9.14

## Preparação

1. Crie uma pasta de execução vazia.
2. Copie `monitora_campsav_alvo_global_v2.9.14.R` para essa pasta.
...
**Source** normalmente. A v2.9.14 preserva a arquitetura homologada na v2.9.1:
...
## Novidades operacionais da v2.9.14

- A incorporação de novas COLETAs em modos `painel_incremental_*` passa a
  preparar o lote no Windows por um processo R isolado e compatível com o
  RStudio, sem o erro de inicialização do processo observado na v2.9.13.
- A busca em `input/novas_coletas/` é recursiva e aceita CSV, XLSX, XLS e ZIP,
  inclusive arquivos em subpastas e ZIPs aninhados.
- Incompletudes e conflitos do lote adicional são diagnosticados antes de
  qualquer alteração da base. Os arquivos de diagnóstico do processo isolado
  são preservados para conferência.
- A edição de UA aceita qualquer valor contratual de `UA-001_VgCS` a
  `UA-999_VgCS`, ainda que a UA não esteja presente no conjunto de dados. O
  mesmo padrão é validado no campo e novamente no servidor antes da fila.
...
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS <- "S"
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS_IMPORTACAO_SISMONITORA <- "S"
MONITORA_OPCAO_REMOVER_UUID_IMPORTACAO_SISMONITORA <- "N" # padrão v2.9.14
```
...
Em lotes de atributos superiores, a v2.9.14 usa o valor bruto exato de cada
