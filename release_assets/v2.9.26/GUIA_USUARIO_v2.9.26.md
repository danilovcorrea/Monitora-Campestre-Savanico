# Guia do usuário — v2.9.26

## Preparação

1. Crie uma pasta de execução vazia.
2. Copie `monitora_campsav_alvo_global_v2.9.26.R` para essa pasta.
...
**Source** normalmente. A v2.9.18 preserva a arquitetura homologada na v2.9.1:
...
- Para gerar o projeto QField, use `MONITORA_OPCAO_GERAR_PROJETO_QFIELD = "S"`
  no bloco operacional. Para incorporar imagens e camadas adicionais, use também
  `MONITORA_OPCAO_IMPORTAR_CAMADAS_QFIELD = "S"`. Em uma execução com uma UC,
  coloque os arquivos diretamente em `qfield_input/`, sem subpasta da UC e sem
  exigir nome específico. O projeto fica em `output/09_qfield/`. Antes do campo,
  teste o ZIP offline no QField e confirme a permissão de uso da imagem.
- O script examina os níveis de zoom realmente materializados no MBTiles:
  zoom máximo 18 ou superior identifica detalhe, 14 ou inferior identifica
  cobertura regional e 15–17 identifica cobertura operacional. O nome do arquivo
  não decide o papel da imagem.
- Para a imagem detalhada, o próprio script deriva um ponto central por UA dos
  consensos aceitos pela validação espacial, cria os círculos de 500 m e recorta
  fisicamente os tiles para WebP com transparência. O MBTiles original permanece
  intocado; hashes, tiles, bordas, descartes e redução ficam auditados no ZIP.
- A mesma pasta `qfield_input/` aceita KML, KMZ, GPKG e ZIP de shapefile. Para
  uma camada linear comum, basta colocar o arquivo na pasta; use
  `camadas_qfield.csv` apenas quando precisar declarar um papel específico.
...
- O relatório analítico trata valores raros, a mudança de medição da
  serrapilheira em 2025 e a sensibilidade da comparação à época da campanha.
  Não modifica automaticamente o conjunto validado nem escolhe uma linha de
  base só pelo aspecto das séries.
- As evidências temporais principais usam as UAs comuns a cada par de anos. Os
  painéis por ano de entrada aparecem como sensibilidade, e não como substitutos
  da população principal. Formas de vida com zero toque não são exibidas nas
  tabelas, embora os produtos tabulares completos permaneçam preservados.
- A inicialização do RStudio e o contrato XLSForm 2025 foram preservados.

### Preservado da v2.9.24

- Exportações XLSX recentes do SISMONITORA podem conter abas adicionais. A aba
  biológica passa a ser reconhecida pelo esquema esperado, sem depender da
  posição da planilha e sem converter silenciosamente textos ou codificação.
- O painel reconcilia representações físicas duplicadas do mesmo campo somente
  quando uma delas está vazia ou quando os valores coincidem. Divergências são
  bloqueadas atomicamente antes de qualquer alteração.
- O relatório de validação separa arquivos atuais e herdados, modificações do
  usuário, modificações automáticas e registros estritamente técnicos de
  auditoria, recuperação e conciliação.
- Os relatórios analíticos explicitam Nº de UAs, Nº de registros e Nº de pontos
  com presença; padronizam contagens, cobertura e proporção relativa; e não
  apresentam comparação temporal artificial quando existe uma única campanha.
- Grafias históricas equivalentes da mesma UC são reconciliadas apenas nas
  cópias internas das estatísticas e relatórios. Dados e linhagem não são
  regravados e nomes materialmente distintos continuam bloqueados.
- CSVs incrementais com aspas escapadas são lidos pelo caminho que o teste de
  execução comprovar seguro. Em ambientes afetados pela duplicação de aspas do
  `data.table::fread`, o script usa o leitor da base R somente nesse ponto e
  registra a decisão na auditoria.

### Preservado da v2.9.23

- Valores `select_one` já materializados como o label exato do XLSForm 2025
  são reconhecidos por meio da relação unívoca `label` → `name` do próprio
  contrato único. A tradução existe somente na cópia usada pela validação; o
  valor observado no dataset e nos relatórios não é regravado.
- `select_multiple` continua estrito. Não foi criado alias, normalização por
  caixa ou acento, mapa local nem flexibilização para tokens históricos.
- Ocorrências de token fora do domínio na mesma linha passam a ser distintas
  por caminho contratual, lista e token inválido. Isso impede colisões entre
  campos diferentes e mantém uma única identidade em todos os relatórios e
  justificativas.

### Preservado da v2.9.22

- Antes de abrir o painel, o script verifica os campos de seleção presentes no
  dataset diretamente contra as `choices` do XLSForm 2025 embutidas no
  contrato único. Tokens fora do domínio aparecem como ocorrência impeditiva;
  o contrato e o XLSForm não são alterados para acomodá-los.
- Correções de hábito são aceitas apenas no campo físico que seja relevante
  para a categoria e a forma de vida daquela linha. Uma tentativa dirigida ao
  ramo errado é bloqueada antes de modificar os dados e, quando inequívoco, o
  relatório indica o ramo contextual aplicável.
- Na mesma célula de lista múltipla, uma substituição de token pode ser seguida
  da inclusão de outro token quando origem, destino e inclusão forem distintos.
  A operação continua atômica; tokens sobrepostos ou ambíguos permanecem
  bloqueados.
- Identificadores estáveis serializados como os textos literais `NA` ou `NULL`
  deixam de ser interpretados como chaves válidas de registro.

### Preservado da v2.9.21

- O preenchimento de um campo textual de “outra espécie exótica” fecha
  automaticamente apenas a cadeia vigente correspondente: `especie=sim`,
  `tipo_forma_vida=exotica`, a forma exótica específica e o token de outra
  espécie na lista imediatamente superior.
- O valor histórico `outros` não é choice válida de `forma_vida_exotica` no
  XLSForm 2025. Em checkpoints antigos, ele só é migrado quando existe uma
  única forma exótica válida já marcada, o texto pode ser preservado
  literalmente e o destino não contém espécie conflitante.
- Se houver mais de uma forma válida, espécie incompatível, texto divergente
  no destino ou outro conflito, o registro permanece inalterado e bloqueado
  para revisão. O contrato não é flexibilizado.
- “Outra forma de vida”, o token histórico `outra` e o token `outros` de
  impactos/manejo são contextos distintos e não acionam essa migração.

### Preservado da v2.9.20

- A composição Sentinel-2 deixa de reatribuir georreferência a visões internas
  do COG. Uma visão interna somente é aceita quando o GDAL preserva bandas,
  CRS e extensão do arquivo oficial; caso contrário, o raster principal é
  utilizado. Caches sem garantia espacial são rejeitados.
- Ao excluir uma correção pendente no painel, o item selecionado é removido
  mesmo quando outras operações da fila continuam conflitantes. Esses
  conflitos permanecem auditados e impedem a aplicação até serem revisados ou
  excluídos.
- A inicialização rápida, os produtos de dados, a linhagem, o contrato único e
  os módulos não relacionados a esses dois escopos permanecem preservados.

### Preservado da v2.9.19

- Na sanitização assistida de hábitos obrigatórios, valores inválidos não
  vazios, como `?`, são agora alvos efetivos da correção quando a ocorrência
  foi confirmada pelo contrato XLSForm.
- Diagnóstico, prévia, aplicação, persistência e replay usam a mesma regra.
  Valores já válidos não são sobrescritos e a reaplicação é idempotente.
- O restante do fluxo operacional permanece igual ao da v2.9.18.

### Preservado da v2.9.18

- O mesmo arquivo único autossuficiente passou a caber com margem no limite
  rígido do editor do RStudio para Windows, inclusive em CRLF. O alerta de
  confirmação para arquivo grande ainda pode aparecer; confirme a abertura e
  use **Source** normalmente.
- O contrato único continua dentro do `.R`, em representação portável
  compactada. Não é necessário fornecer XLSForm, tabela de domínio, projeto,
  launcher ou qualquer outro arquivo auxiliar.
- Regras, opções, interface, linhagem e produtos são os mesmos da v2.9.17.

### Preservado da v2.9.17

- O relatório consolidado de validação pode ser materializado em Markdown e
  DOCX editáveis, além dos formatos já disponíveis. O DOCX usa modelo A4
  embutido, tabelas legíveis e hyperlinks, sem depender de arquivo local.
- Os achados analíticos identificam o ano inicial e o número de UAs da
  população comparada e mostram como a direção observada se comporta nos
  painéis iniciados após ampliações do esforço amostral.
- O fechamento hierárquico de formas de vida e o reconhecimento dos contextos
  de impactos usam exclusivamente paths, names, labels, aliases e dependências
  do contrato único. Texto livre não é convertido em token.
- Campo contextual vazio, atributo ausente e resolução contratual ambígua são
  situações distintas nos relatórios; ambiguidade nunca é apresentada como
  ausência declarada de contexto.
- A geração documental usa caminho temporário curto e publicação verificada,
  reduzindo o risco de caminhos longos no Word para Windows. Falhas documentais
  permanecem isoladas dos produtos de dados.

### Preservado da v2.9.16

- A busca Sentinel-2 percorre as páginas do catálogo e amplia a janela temporal
  dentro de limites definidos. Cobertura da área, baixa nebulosidade local e
  recência continuam orientando a seleção.
- Quando a aquisição ideal não está disponível, a rotina tenta, em ordem
  auditável, a melhor aquisição integral, uma composição temporal e o último
  mapa validado da mesma UC. O mapa não é omitido apenas por qualidade inferior
  ao alvo preferencial.
- Data, qualidade, cobertura, paginação, modo de composição e eventual uso de
  contingência constam na auditoria e na informação editorial pertinente.
- Nas continuações incrementais, o manifesto passa a conferir também o hash e
  a quantidade de linhas de `inventario_sessoes_linhagem.csv`.
- A ordem das sessões já registrada é preservada, inclusive quando uma execução
  legada não possui data ou decisões semânticas. A sessão atual é acrescentada
  ao final e o inventário resultante recebe nova assinatura.
- O inventário não substitui o ledger e nunca cria eventos artificiais. Ele
  registra a existência das execuções; as decisões continuam exclusivamente na
  trilha semântica.
- As funcionalidades consolidadas nas versões anteriores — relatórios, painel,
  replay, incorporação de COLETAs, edição de UA, cartografia oficial e gates
  documentais — permanecem preservadas.

As revisões não alteram o contrato único, os produtos de dados nem as regras de
validação já consolidadas.
...
## Incorporação de COLETAs omitidas

Se uma continuidade incremental precisar receber COLETAs que não estavam na
entrada original, não reinicie a curadoria e não una CSVs manualmente:

1. Copie para `input/` o `registros_corrig.csv` e a pasta `linhagem/` da mesma
   execução anterior.
2. Use um modo `painel_incremental_*`.
3. Crie `input/novas_coletas/` e coloque ali somente os arquivos brutos
   adicionais.
4. Defina `MONITORA_OPCAO_INCORPORAR_NOVAS_COLETAS <- "S"`.

A pesquisa é recursiva: os arquivos podem estar em subpastas. São aceitos CSV,
XLSX, XLS e ZIP; ZIPs aninhados são extraídos pelo próprio script. Não misture
nessa pasta produtos de dados ou arquivos pertencentes a outra cadeia de
curadoria.

O lote é atômico. COLETAs novas e completas são incorporadas; repetições
factualmente idênticas são ignoradas; conflito de conteúdo, UUID repetido ou
incompletude bloqueia toda a incorporação antes de alterar o checkpoint. A
operação é idempotente e registrada em
`output/02_painel_correcoes/linhagem/incorporacoes_novas_coletas.csv`. As novas
linhas entram no painel sem herdar por hipótese as correções manuais aplicadas
a registros históricos. Se a preparação isolada detectar uma COLETA
incompleta, a mensagem identifica os pontos ausentes e a incorporação inteira
permanece sem efeito. Os diagnósticos são publicados na auditoria da execução
antes da limpeza dos arquivos temporários.
...
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS <- "S"
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS_IMPORTACAO_SISMONITORA <- "S"
MONITORA_OPCAO_REMOVER_UUID_IMPORTACAO_SISMONITORA <- "N" # padrão v2.9.18
```
...
Para corrigir a unidade amostral, informe o valor completo no padrão
`UA-XXX_VgCS`. São aceitos `UA-001_VgCS` a `UA-999_VgCS`, mesmo quando o valor
ainda não aparece na lista de UAs observadas. Abreviações como `004`, diferenças
de maiúsculas/minúsculas e valores fora desse intervalo são bloqueados antes da
fila. A correção de UA continua sendo um atributo superior e alcança todas as
...
Em lotes de atributos superiores, a v2.9.18 usa o valor bruto exato de cada
...
A v2.9.18 aplica a mesma proteção ao arquivo
`inventario_sessoes_linhagem.csv`. O manifesto confere o hash e a quantidade de
sessões antes de qualquer continuidade. A ordem herdada é mantida exatamente,
inclusive para execuções antigas sem data; a execução atual é acrescentada ao
final. Por isso, não edite o inventário e não copie arquivos de linhagem de
rodadas diferentes.
...
altera. O botão geral **Limpar filtros**, na barra lateral, reinicia os campos
de todas as abas, inclusive buscas e seleções das tabelas, sem remover
operações já adicionadas às filas. O botão local **Limpar filtros espaciais**
reinicia somente filtros, COLETAS, coordenadas e campos da aba espacial.
