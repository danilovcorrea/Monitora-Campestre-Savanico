# Guia do usuário — v2.9.8

## Preparação

1. Crie uma pasta de execução vazia.
2. Copie `monitora_campsav_alvo_global_v2.9.8.R` para essa pasta.
...
**Source** normalmente. A v2.9.8 preserva a arquitetura homologada na v2.9.1:
...
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS <- "S"
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS_IMPORTACAO_SISMONITORA <- "S"
MONITORA_OPCAO_REMOVER_UUID_IMPORTACAO_SISMONITORA <- "N" # padrão v2.9.8
```
...
Em lotes de atributos superiores, a v2.9.8 usa o valor bruto exato de cada
COLETA como precondição. Checkpoints produzidos pela candidata inicial da
v2.9.8 só são ajustados automaticamente quando a divergência se limita a espaços
nas extremidades e a impressão digital comprova que a base é a mesma. O ajuste
é auditado e nunca corrige silenciosamente uma divergência material.
...
executar uma recuperação única a partir dos sidecars das rodadas que formam a
...
propagada para as rodadas seguintes. A bolsista passa a usar somente o fluxo
...
`output/02_painel_correcoes/linhagem/` e na pasta `dados/` do relatório.
