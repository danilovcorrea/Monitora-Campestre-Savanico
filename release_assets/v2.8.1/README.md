...
- Versão: `v2.8.1`
- Script principal: [`monitora_campsav_alvo_global_v2.8.1.R`](monitora_campsav_alvo_global_v2.8.1.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.8.1/`](releases/v2.8.1/)
- Notas da versão: [`RELEASE_NOTES_v2.8.1.md`](RELEASE_NOTES_v2.8.1.md)
- Release no GitHub: [v2.8.1](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.8.1)
...
- Fechamento hierárquico do TRIOUT passa a resolver resíduos legados de “outra forma de vida” no estado final materializado, inclusive após operações concorrentes da mesma fila.
- Persistência e gate pós-exportação reconfirmam que a sanitização efetiva permaneceu aplicada depois da releitura do checkpoint.
- Produtos KML e KMZ redesenhados por finalidade: camada operacional para a equipe de campo, camada estatística restrita e áreas de proteção de 100 m.
- KML operacional sem dados primários da amostragem, preservando `form_veg`, metadados espaciais, simbologia e rótulos das UAs.
- Áreas operacionais sem preenchimento, delimitadas em amarelo e identificadas pelo rótulo da UA; o ano usado como referência espacial fica registrado nos atributos.
- Validação espacial guiada com seleção de UC, UA e ano de origem e destino, preenchimento das coletas compatíveis e recomendações determinísticas auditáveis.
- Sanitização espacial limitada a casos inequívocos, sem substituir decisões que dependem de interpretação humana.
- Precedência bidirecional: correções específicas permanecem protegidas independentemente de serem registradas antes ou depois de SANHAB, SANEORF, TRIOUT, movimentos em lote ou sanitizações espaciais.
- Movimentos atômicos limpam as dependências contratuais da categoria de origem antes da reconciliação, evitando reintrodução do token removido.
- Triagem espacial incremental e relatórios de validação preservam todas as coletas e a linhagem entre execuções.
- Compatibilidade preservada com o contrato consolidado, os 13 modos públicos, o replay semântico e os gates de materialização.
...
2. Copie `monitora_campsav_alvo_global_v2.8.1.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.8.1.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.8.1 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.8.1.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.8.1.md`: notas da versão atual.
- `GUIA_USUARIO_v2.8.1.md`: roteiro operacional resumido.
- `release_assets/v2.8.1/`: conjunto mínimo de artefatos da release.
- `releases/v2.8.1/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.8.1. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.8.1. Acesso em: DD mês AAAA.
```
