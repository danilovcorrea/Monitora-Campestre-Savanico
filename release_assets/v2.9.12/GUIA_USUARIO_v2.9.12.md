# Guia do usuário — v2.9.12

## Preparação

1. Crie uma pasta de execução vazia.
2. Copie `monitora_campsav_alvo_global_v2.9.12.R` para essa pasta.
...
**Source** normalmente. A v2.9.12 preserva a arquitetura homologada na v2.9.1:
...
## Novidades operacionais da v2.9.12

- O mapa Sentinel representa o limite oficial da UC no mapa principal quando
  o contorno intercepta a moldura cartográfica. Quando o limite não aparece na
  escala das UAs, ele permanece somente no localizador e não ocupa a legenda
  principal.
- A moldura e os rótulos de coordenadas do mapa passam a usar a mesma largura
  editorial da faixa inferior com localizador, legenda, informações e marcas.
  A proporção do raster e a posição das UAs permanecem preservadas.
- O botão geral **Limpar filtros** reinicia filtros, campos transitórios,
...
- A limpeza não apaga o responsável pela sessão, correções, operações
  espaciais, justificativas já adicionadas, históricos ou auditorias.
- O botão **Limpar filtros espaciais** continua restrito à aba espacial e não
  altera campos ou seleções das demais abas.

Permanecem as revisões da v2.9.11: relatórios transversais para campanha única,
busca Sentinel-2 temporalmente progressiva, localizador resiliente, persistência
transacional, rótulo legível de `amostragem/especie` e prévia integral otimizada.
...
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS <- "S"
MONITORA_OPCAO_GERAR_REGISTROS_VALIDADOS_IMPORTACAO_SISMONITORA <- "S"
MONITORA_OPCAO_REMOVER_UUID_IMPORTACAO_SISMONITORA <- "N" # padrão v2.9.12
```
...
Em lotes de atributos superiores, a v2.9.12 usa o valor bruto exato de cada
