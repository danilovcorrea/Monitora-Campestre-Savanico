...
- Versão: `v2.9.1`
- Script principal: [`monitora_campsav_alvo_global_v2.9.1.R`](monitora_campsav_alvo_global_v2.9.1.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.1/`](releases/v2.9.1/)
- Notas da versão: [`RELEASE_NOTES_v2.9.1.md`](RELEASE_NOTES_v2.9.1.md)
- Release no GitHub: [v2.9.1](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.1)
...
- A planilha SISMONITORA ganha uma opção independente para esvaziar ou
  preservar `uuid` e `amostragem/registro/uuid`; a fonte validada nunca é
  alterada e o padrão homologado para inclusão de registros novos é preservado.
- Os relatórios sintético e detalhado passam a incluir DOCX, além de Rmd,
  Markdown, HTML e PDF, e adotam títulos editoriais harmonizados.
- O mapa Sentinel-2 adota o leiaute cartográfico quadrado aprovado: área
  principal superior e faixa institucional inferior com localizador, legenda,
  informações, fontes, elaboração e marcas Monitora, CBC e ICMBio.
- Escala gráfica e numérica, norte geográfico, moldura, coordenadas, grade,
  projeção, período, fonte e limitações ficam explicitamente documentados.
- A extensão, projeção, imagem, limites da UC, bioma e estados são resolvidos a
  partir do dataset, sem hardcode de UC; arquivos oficiais baixados para o
  localizador são temporários e removidos após a materialização em memória.
- O vocabulário cartográfico passa a usar “Continuidade do esforço amostral nas
  UAs” e “Formação vegetacional”, inclusive no mapa vetorial complementar.
- A baseline v2.9.0, o contrato único, o painel, os 13 modos, a linhagem, o
  replay, os produtos centrais, estatísticas e KML/KMZ permanecem preservados.
- Relatórios, mapa orbital e planilhas de importação permanecem desligados por
  padrão e não materializam custo próprio quando configurados como `N`.
...
2. Copie `monitora_campsav_alvo_global_v2.9.1.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.1.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.1 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.1.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.1.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.1.md`: roteiro operacional resumido.
- `release_assets/v2.9.1/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.1/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.1. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.1. Acesso em: DD mês AAAA.
```
