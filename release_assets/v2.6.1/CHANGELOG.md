...
- Painel de correções assistidas: atributos administrativos e contratuais do template 2025 disponíveis ao bolsista quando editáveis.
- Contrato de dados: `registros_corrig.csv` passa a concentrar saneamento, sincronização de aliases, validação e auditoria dos 129 atributos do template XLSForm/SISMONITORA 2025.
- Produto validado: `registros_validados.csv` passa a ser derivado apenas por seleção e reordenação dos atributos do template quando não houver pendências impeditivas.
- Relatório de validação: HTML como formato padrão público, com auditorias de bloqueio mais claras.
- Governança: defaults públicos seguros preservados, sem abertura automática do painel e sem geração local de produtos sensíveis por padrão.
