...
- Versão: `v2.9.0`
- Script principal: [`monitora_campsav_alvo_global_v2.9.0.R`](monitora_campsav_alvo_global_v2.9.0.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.0/`](releases/v2.9.0/)
- Notas da versão: [`RELEASE_NOTES_v2.9.0.md`](RELEASE_NOTES_v2.9.0.md)
- Release no GitHub: [v2.9.0](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.0)
...
- Relatórios analíticos opcionais por UC, em versões sintética e detalhada, com
  Rmd, Markdown, HTML, PDF, tabelas e figuras editáveis.
- Esforço amostral descrito por UC, formação vegetacional e ano, com número
  efetivamente observado de UAs e pontos amostrais, continuidade temporal,
  situação dos dados e evidências priorizadas.
- Fundo Sentinel-2 L2A opcional, público e recente, selecionado pela extensão
  do dataset sem conta, chave, token, faturamento ou hardcode de UC.
- Planilhas SISMONITORA particionadas automaticamente por UC + ciclo + campanha
  quando `registros_validados.csv` contém múltiplos contextos.
- PDF e finalização resilientes no Windows, Linux e macOS, com autodetecção de
  navegador e organização canônica do output antes de qualquer bloqueio final.
- Gates por token, cache semanticamente invalidável e checkpoints granulares
  reduzem varreduras contratuais desnecessárias sem enfraquecer auditorias.
- Inicialização imediata no RStudio mesmo se `Source with Echo` estiver ativo;
  o programa continua legível, autossuficiente e avaliado no ambiente global.
- A baseline v2.8.2, o contrato único, o painel, os 13 modos, a linhagem, o
...
2. Copie `monitora_campsav_alvo_global_v2.9.0.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.0.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.0 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.0.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.0.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.0.md`: roteiro operacional resumido.
- `release_assets/v2.9.0/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.0/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.0. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.0. Acesso em: DD mês AAAA.
```
