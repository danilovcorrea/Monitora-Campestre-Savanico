...
- Versão: `v2.7.3`
- Script principal: [`monitora_campsav_alvo_global_v2.7.3.R`](monitora_campsav_alvo_global_v2.7.3.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.7.3/`](releases/v2.7.3/)
- Notas da versão: [`RELEASE_NOTES_v2.7.3.md`](RELEASE_NOTES_v2.7.3.md)
- Release no GitHub: [v2.7.3](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.7.3)
...
- Projeção de hábitos em `registros_validados.csv` a partir do atributo canônico já reconciliado de `registros_corrig.csv`, usando a mesma regra contratual do diagnóstico e do SANHAB.
- Reconciliação adicional de aliases por token contratual e equivalência segura de valores escalares repetidos em campos históricos separados por `|`.
- Proteção de atributos canônicos durante a limpeza automática de dependências condicionais.
- Preservação integral do painel, da fila, dos atributos editáveis, das operações semânticas e dos 13 modos públicos.
- Validação estratificada aprovada nos 13 modos e em 22 UCs, com 626 gates operacionais aprovados e nenhuma falha.
...
2. Copie `monitora_campsav_alvo_global_v2.7.3.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.7.3.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.7.3 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.7.3.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.7.3.md`: notas da versão atual.
- `GUIA_USUARIO_v2.7.3.md`: roteiro operacional resumido.
- `release_assets/v2.7.3/`: conjunto mínimo de artefatos da release.
- `releases/v2.7.3/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.7.3. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.7.3. Acesso em: DD mês AAAA.
```
