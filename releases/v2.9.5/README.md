...
- Versão: `v2.9.5`
- Script principal: [`monitora_campsav_alvo_global_v2.9.5.R`](monitora_campsav_alvo_global_v2.9.5.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.5/`](releases/v2.9.5/)
- Notas da versão: [`RELEASE_NOTES_v2.9.5.md`](RELEASE_NOTES_v2.9.5.md)
- Release no GitHub: [v2.9.5](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.5)
...
- A triagem de vegetação seca ou morta passa a orientar a identificação de
  possíveis falsos positivos e exibe COLETA, UA, ano, forma de vida, grupo
  herbáceo/lenhoso, trajetória e contexto de impactos e manejo.
- Em **Justificar pendências**, a tabela respeita os filtros, todo o conjunto
  visível pode ser selecionado e as justificativas da sessão podem ser excluídas
  individualmente ou em lote. Inclusões e exclusões são atômicas.
- A ordem operacional das abas foi restaurada e o movimento assistido de exótica
  para nativa foi verificado com preservação de hábito e descritores compatíveis.
- O localizador usa referências oficiais temporárias, sem incorporar shapefile,
  hardcode de UC ou artefato espacial persistente.
- Relatórios compactos, Sentinel-2 público, símbolos estatísticos auditados,
  contrato único, 13 modos, linhagem, replay, produtos SISMONITORA, KML/KMZ e
  cautela causal da v2.9.4 permanecem preservados.
...
2. Copie `monitora_campsav_alvo_global_v2.9.5.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.5.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.5 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.5.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.5.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.5.md`: roteiro operacional resumido.
- `release_assets/v2.9.5/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.5/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.5. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.5. Acesso em: DD mês AAAA.
```
