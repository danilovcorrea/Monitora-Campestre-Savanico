...
- Versão: `v2.9.6`
- Script principal: [`monitora_campsav_alvo_global_v2.9.6.R`](monitora_campsav_alvo_global_v2.9.6.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.6/`](releases/v2.9.6/)
- Notas da versão: [`RELEASE_NOTES_v2.9.6.md`](RELEASE_NOTES_v2.9.6.md)
- Release no GitHub: [v2.9.6](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.6)
...
- O relatório de validação passa a inventariar explicitamente as execuções e as
  sessões da cadeia, distinguindo eventos herdados, reaplicados, novos e
  acumulados, sem confundir rodada sem decisão nova com sessão semântica.
- A aba **Justificar pendências** reconcilia ocorrências após correções, mantém
  lotes sucessivos e disjuntos selecionáveis e aplica inclusões, exclusões e
  encerramento como uma única transação lógica auditável.
- Falhas na planilha opcional de importação do SISMONITORA deixam de interromper
  a materialização dos demais produtos; o erro permanece registrado e o XLSX
  afetado não é declarado concluído.
- A geração dos relatórios analíticos informa o progresso de catálogo Sentinel,
  aquisição, mosaico, mapa, DOCX, HTML e PDF. A impressão PDF ocorre em processo
  isolado, evitando mensagens técnicas de encerramento do navegador no console.
- A inicialização rápida homologada na v2.9.1 permanece intocada. No teste real
  pelo botão **Source** do RStudio para Windows, a primeira expressão iniciou
  0,743 s após o comando do IDE.
...
Os padrões da v2.9.6 representam a continuidade incremental integral usada pela
...
O relatório consolidado materializa `inventario_sessoes_linhagem.csv`, que
distingue execuções registradas, sessões com decisões, eventos herdados,
reaplicados, novos e acumulados.
...
2. Copie `monitora_campsav_alvo_global_v2.9.6.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.6.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.6 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.6.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.6.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.6.md`: roteiro operacional resumido.
- `release_assets/v2.9.6/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.6/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.6. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.6. Acesso em: DD mês AAAA.
```
