...
- Versão: `v2.9.3`
- Script principal: [`monitora_campsav_alvo_global_v2.9.3.R`](monitora_campsav_alvo_global_v2.9.3.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.3/`](releases/v2.9.3/)
- Notas da versão: [`RELEASE_NOTES_v2.9.3.md`](RELEASE_NOTES_v2.9.3.md)
- Release no GitHub: [v2.9.3](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.3)
...
- A equipe da COLETA passa a ser editada como repeat: cada nome ocupa uma linha,
  o CPF correspondente permanece na mesma posição e continua opcional.
- Formatos históricos de coletores são saneados automaticamente antes do painel;
  CPF só é preservado quando a associação com o nome é inequívoca.
- `impact_manejo_uso` e `tipos_impacto_manejo_uso` exibem `label — name`, respeitam
  a dependência Sim/Não e aplicam operações auditáveis por token.
- A aba **Justificar pendências** aceita seleção múltipla e passa a incluir
  pendências e alertas espaciais remanescentes, sem alterar dados nem liberar
  gates impeditivos.
- Vegetação seca ou morta recebe diagnóstico não impeditivo e relatórios próprios,
  preservando a separação entre ocorrência observada e hipótese ecológica.
- Os relatórios analíticos incorporam cobertura e proporção relativa da estrutura
  herbácea/lenhosa, categorias gerais, formas nativas, exóticas, secas ou mortas e
  material botânico, com testes pareados, IC95%, FDR-BH, equivalência e mudança de
  composição.
- Médias anuais exibem símbolos inferenciais derivados dos testes existentes;
  textos e tabelas foram compactados editorialmente sem alterar CSVs ou resultados.
- O contrato único, os 13 modos, defaults, linhagem, replay, produtos centrais,
  XLSX SISMONITORA, KML/KMZ e cartografia homologada da v2.9.2 são preservados.
...
2. Copie `monitora_campsav_alvo_global_v2.9.3.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.3.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.3 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.3.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.3.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.3.md`: roteiro operacional resumido.
- `release_assets/v2.9.3/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.3/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.3. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.3. Acesso em: DD mês AAAA.
```
