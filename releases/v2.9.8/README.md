...
- Versão: `v2.9.8`
- Script principal: [`monitora_campsav_alvo_global_v2.9.8.R`](monitora_campsav_alvo_global_v2.9.8.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.8/`](releases/v2.9.8/)
- Notas da versão: [`RELEASE_NOTES_v2.9.8.md`](RELEASE_NOTES_v2.9.8.md)
- Release no GitHub: [v2.9.8](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.8)
...
- Uma falha de precondição interrompe imediatamente a prévia atômica e informa
  operação, COLETA, atributo, status, causa e auditoria detalhada.
- Filas idênticas que já falharam não são recalculadas repetidamente.
- O checkpoint recuperável preserva em conjunto correções de campos, operações
  espaciais, justificativas, histórico de intenções e auditorias e só é
  restaurado automaticamente sobre a mesma base.
- Lotes de atributos superiores passam a conservar o valor bruto exato de cada
  COLETA como precondição, separado do texto amigável exibido no painel.
- Checkpoints afetados pela candidata anterior podem ser reidratados somente
  quando a divergência se limita a espaços nas extremidades; divergências
  materiais continuam bloqueadas.
- O relatório de validação usa corretamente o genérico `unique` do R.
...
Os padrões da v2.9.8 representam a continuidade incremental integral usada pela
...
2. Copie `monitora_campsav_alvo_global_v2.9.8.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.8.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.8 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.8.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.8.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.8.md`: roteiro operacional resumido.
- `release_assets/v2.9.8/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.8/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.8. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.8. Acesso em: DD mês AAAA.
```
