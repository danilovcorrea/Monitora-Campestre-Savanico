...
- Versão: `v2.9.4`
- Script principal: [`monitora_campsav_alvo_global_v2.9.4.R`](monitora_campsav_alvo_global_v2.9.4.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.4/`](releases/v2.9.4/)
- Notas da versão: [`RELEASE_NOTES_v2.9.4.md`](RELEASE_NOTES_v2.9.4.md)
- Release no GitHub: [v2.9.4](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.4)
...
- Os relatórios analíticos foram condensados sem remover resultados técnicos:
  na homologação FNCS, o sintético passou de 22 para 9 páginas e o detalhado
  de 36 para 16 páginas.
- O sintético prioriza seis figuras executivas e o detalhado dez figuras
  analíticas; os 104 PNGs, CSVs e testes estatísticos permanecem completos e
  auditados nos produtos.
- Quando os relatórios são ativados, o mapa usa por padrão imagem Sentinel-2
  pública, recente, sem conta, chave, token, faturamento ou hardcode de UC.
- Com os relatórios desligados, nenhuma dependência, consulta orbital, leitura de
  imagem ou etapa cartográfica específica é iniciada.
- O script passa a ser avaliado diretamente no RStudio, eliminando a espera de
  vários minutos que podia ocorrer antes da primeira mensagem no Windows.
- O Pandoc do RStudio ou do Quarto passa a ser resolvido automaticamente no
  Windows, Linux e macOS quando os formatos documentais o exigem.
- A v2.9.3 permanece integralmente preservada quanto ao contrato único, aos 13
  modos, painel, sanitizações, justificativas, linhagem, replay, produtos de
  dados, SISMONITORA, estatísticas, KML/KMZ e cautela causal.
...
2. Copie `monitora_campsav_alvo_global_v2.9.4.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.4.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.4 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.4.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.4.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.4.md`: roteiro operacional resumido.
- `release_assets/v2.9.4/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.4/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.4. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.4. Acesso em: DD mês AAAA.
```
