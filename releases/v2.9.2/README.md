...
- Versão: `v2.9.2`
- Script principal: [`monitora_campsav_alvo_global_v2.9.2.R`](monitora_campsav_alvo_global_v2.9.2.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.2/`](releases/v2.9.2/)
- Notas da versão: [`RELEASE_NOTES_v2.9.2.md`](RELEASE_NOTES_v2.9.2.md)
- Release no GitHub: [v2.9.2](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.2)
...
- A edição de atributos superiores passa a respeitar o escopo real da COLETA:
  campos de seleção múltipla operam por token e campos textuais podem substituir,
  acrescentar ou limpar conteúdo sem perder o valor anterior por engano.
- Movimentos entre formas de vida fazem preflight da subárvore completa e só
  escrevem quando lista, hábito, espécies e descritores podem ser transferidos
  atomicamente; uma falha não deixa estado parcial.
- A validação espacial reúne os fluxos por COLETA e por ANO em uma única seção,
  remove contagens esperadas do formulário, preenche destino inequívoco e limpa
  filtros, COLETA e coordenadas sem resíduos.
- Mudanças de formação vegetacional na mesma UA passam a gerar ocorrência
  diagnóstica não impeditiva e relatório próprio para revisão.
- A nova aba de justificativas registra responsável, classificação, texto,
  timestamp e ID estável da pendência em trilha append-only. Justificar não
  corrige o dado nem libera gates impeditivos.
- Quando o mapa Sentinel é concluído, o mapa vetorial equivalente é suprimido;
  o mapa por ano permanece. Título, mapa, barra institucional e texto usam a
  mesma largura editorial nos cinco formatos dos relatórios.
- O contrato único, os 13 modos, os defaults, a linhagem, o replay, os produtos
  centrais, as estatísticas, os KML/KMZ e os módulos opcionais da v2.9.1 são
  preservados.
...
2. Copie `monitora_campsav_alvo_global_v2.9.2.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.2.R`.
6. Confira os produtos em `output/`, a documentação em `docs/` e a trilha técnica em `log/`.

## Modos de execução

A v2.9.2 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.2.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.2.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.2.md`: roteiro operacional resumido.
- `release_assets/v2.9.2/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.2/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.2. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.2. Acesso em: DD mês AAAA.
```
