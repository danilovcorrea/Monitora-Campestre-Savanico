# Guia do usuário — v2.9.2

## Preparação

1. Crie uma pasta de execução vazia.
2. Copie `monitora_campsav_alvo_global_v2.9.2.R` para essa pasta.
...
**Source** normalmente. A v2.9.2 preserva o encapsulamento interno que evita a
fila de milhares de expressões causada por `Source with Echo` e desativa
persistentemente essa preferência quando necessário. Em homologação real no
RStudio para Windows, a primeira expressão começou em menos de um segundo após
o clique em **Source**; a abertura inicial do arquivo grande continua sendo uma
etapa própria do editor.
...
além de tabelas e figuras editáveis de
...
ocorrências que permaneçam ao fim da revisão. Cada evento recebe responsável,
classificação, timestamp e ID estável. A justificativa é auditável, mas não
corrige o dado, não encerra a ocorrência por decisão textual e não libera uma
