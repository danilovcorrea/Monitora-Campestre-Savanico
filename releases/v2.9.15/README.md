...
- Versão: `v2.9.15`
- Script principal: [`monitora_campsav_alvo_global_v2.9.15.R`](monitora_campsav_alvo_global_v2.9.15.R)
- Script canônico: [`monitora_campsav_alvo_global.R`](monitora_campsav_alvo_global.R)
- Cópia congelada: [`releases/v2.9.15/`](releases/v2.9.15/)
- Notas da versão: [`RELEASE_NOTES_v2.9.15.md`](RELEASE_NOTES_v2.9.15.md)
- Release no GitHub: [v2.9.15](https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.15)
...
## Destaques da v2.9.15

- Subconjuntos analíticos vazios, inclusive por ausência de uma formação
  vegetacional, passam a manter schema estável e não interrompem os relatórios.
- Estados e biomas são obtidos das edições oficiais mais recentes do IBGE; o
  limite de UC federal permanece no serviço oficial ICMBio/INDE. Fonte, edição,
  escala, atualização, checksum e eventual fallback ficam auditados.
- Operações concorrentes sobre uma mesma ocorrência `desconhecida` são
  reconhecidas pelo alvo semântico e bloqueadas antes da mutação.
- O replay aceita acréscimos textuais históricos, repara em memória ledgers v2
  anteriores e permanece idempotente, sem duplicar o fragmento reaplicado.
- O DOCX passa a validar cada figura esperada no OOXML; mapa Sentinel, legendas,
  mídia e relacionamentos ausentes impedem a aprovação do documento incompleto.
- Painéis `evidencia_estatistica_*.png` disponíveis são incorporados uma única
  vez, dentro do tema correspondente no relatório detalhado, sem recalcular os
  testes e sem ampliar o relatório sintético.
- A capa do DOCX reproduz a identidade do PDF/HTML, com hierarquia institucional,
  marcas separadas fisicamente, título e metadados completos; páginas internas
  preservam cabeçalho, rodapé e numeração.
...
Os padrões da v2.9.15 representam a continuidade incremental integral usada pela
...
2. Copie `monitora_campsav_alvo_global_v2.9.15.R` para a raiz.
...
5. Execute no RStudio ou com `Rscript monitora_campsav_alvo_global_v2.9.15.R`.
...
A v2.9.15 preserva os 13 modos públicos:
...
- `monitora_campsav_alvo_global_v2.9.15.R`: script versionado atual.
...
- `RELEASE_NOTES_v2.9.15.md`: notas da versão atual.
- `GUIA_USUARIO_v2.9.15.md`: roteiro operacional resumido.
- `release_assets/v2.9.15/`: conjunto mínimo de artefatos da release.
- `releases/v2.9.15/`: cópia congelada da versão.
...
```text
CORRÊA, Danilo V. Monitora Campestre-Savânico — Alvo Global. Versão v2.9.15. GitHub, 2026. Disponível em: https://github.com/danilovcorrea/Monitora-Campestre-Savanico/releases/tag/v2.9.15. Acesso em: DD mês AAAA.
```
