...
### 6 - Os gráficos gerados podem ser exportados em formato .png ou .pregistros_corrig_stat no menu "Export" na aba "gráficos";
...
# Percorre cada arquivo .xlsx.
for (xlsx_file in xlsx_files) {
  # Cria o nome correspondente do arquivo .csv.
  csv_file <- sub("\\.xlsx$", ".csv", xlsx_file)
  
  # verificação se o arquivo .csv já existe
  if (!file.exists(csv_file)) {
    # Lê o arquivo .xlsx como tabela de dados
    data <- read_excel(xlsx_file)
    
    # Grava a tabela de dados em um arquivo .csv
...
csvfiles <-
  ...
  stringr::str_subset(., "cob_veg_categ.csv", negate = TRUE) %>%
  stringr::str_subset(., "cob_veg_form_vida_exot.csv", negate = TRUE) %>%
  stringr::str_subset(., "cob_veg_form_vida_nat.csv", negate = TRUE) %>%
  stringr::str_subset(., "cob_veg_form_vida_seca_morta.csv", negate = TRUE) %>%
  stringr::str_subset(., "cob_veg_herb_lenh.csv", negate = TRUE) %>%
  stringr::str_subset(., "prop_rel_categ.csv", negate = TRUE) %>%
  stringr::str_subset(., "prop_rel_form_vida_exot.csv", negate = TRUE) %>%
  stringr::str_subset(., "prop_rel_form_vida_nat.csv", negate = TRUE) %>%
  stringr::str_subset(., "prop_rel_form_vida_seca_morta.csv", negate = TRUE) %>%
  stringr::str_subset(., "prop_rel_herb_lenh.csv", negate = TRUE)
...
# verificação if '.id' exists in the 'registros' data.table

if (!".id" %in% colnames(registros)) {
  # Se '.id' não existir, cria a coluna com NA.
  registros[, .id := NA]
}
...
### Renomeia colunas do XLSX baixado pelo script para compatibilidade com o CSV do SISMONITORA.
...
### Consolida nomes de colunas duplicadas.

# Identifica nomes de colunas duplicados.
dups <- names(registros_corrig)[duplicated(names(registros_corrig))]

# Percorre cada nome de coluna duplicado e consolida valores não ausentes.
for (col in unique(dups)) {
  # Obtém os índices das colunas chamadas `col`.
  ...
  # Remove as colunas duplicadas de mesmo nome.
  registros_corrig[, (cols[-1]) := NULL]
}

rm(col, cols, dups)

### Renomeia valores de colunas do XLSX baixado pelo script para compatibilidade com o CSV do SISMONITORA.
...
### correção de rótulos para nomes
...
### Corrige delimitadores incorretos nas coordenadas.
...
### Remove colunas auxiliares com valores NA.
...
### criação sum_nativa, sum_exotica, sum_seca_morta e remove colunas auxiliares nativa, exotica e seca_morta:
...
# Colunas de formas de vida de plantas.
...
# Coluna de categorias que encostam na vareta.
...
# Junção à esquerda da presença de nativas, exóticas e secas/mortas.
registros_corrig_stat <- registros_corrig_stat %>%
...
#relocate serrapilheira, solo_nu (any_of)
registros_corrig_stat <- registros_corrig_stat %>%
  relocate(any_of(c("serrapilheira", "solo_nu")),
           .before = sum_herbacea)
...
> 0) {
  ...
  plot_p1.1.1_prop_rel_herb_lenh_camp_sem_rotulo <- reg_corrig_stat_summarise_p1 %>%
    ...
    theme_bw() # +
  # theme(gráfico.title.position = "gráfico",
  # gráfico.title = ggtext::element_textbox_simple())
...
## gráfico nativa, exotica, seca_morta, serrapilheira, solo_nu
...
### gráfico formas vida seca ou morta
...
# Remove linhas com coordenadas ausentes, quando houver.
...
# Cria uma geometria LineString para cada linha, combinando as coordenadas inicial e final.
...
# Cria uma nova tabela de dados para os pontos iniciais e finais, preservando todos os atributos originais.
...
# Combina os data frames de pontos e linhas.
...
# Atribui o SRC WGS84 - EPSG:4326 ao objeto sf.
combined_sf_object <- st_set_crs(combined_sf_object, 4326)

# Exporta para arquivo KML, sobrescrevendo se já existir.
